...
if (!WE) { console.error('找不到 Wallpaper Engine 安装目录；用 DSH_WE_WE 指定后重跑。'); process.exit(1); }
const WS = [
  process.env.DSH_WE_WS,
...
if (!WS) { console.error('找不到 Wallpaper Engine workshop 库；用 DSH_WE_WS 指定后重跑。'); process.exit(1); }
const cases = [
...
for (const [id, stem] of cases) {
  const r = new SceneRenderer(WS + '/' + id + '/scene.pkg', { width: 100, height: 100, time: 0, weAssetsDir: WE, log: () => {} });
