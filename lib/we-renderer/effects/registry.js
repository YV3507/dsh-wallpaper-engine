...
export const effectRegistry = {
  ...
  lightshafts(ctx) { return this.effectLightshafts(ctx.img, ctx.c, ctx.t, ctx.pass, ctx.ef, ctx.name); },
  ...
  godrays(ctx) { return this.effectGodrays(ctx.img, ctx.passes, ctx.t, ctx.ef, ctx.name); },
  ...
  // blurprecise: 无手写内核 —— 按壁纸自带的 effect.json 数据执行
  // (effects/blurprecise/effect.json → materials/effects/blur_precise_gaussian_{x,y}.json →
  //  shaders/effects/blur_precise_gaussian.frag|vert; 旧条目是无条件 no-op, 已删除)
  blurprecise(ctx) { return this._applyEffectJsonEffectScaled(ctx.img, ctx.ef, ctx.name, ctx.t); },
};
