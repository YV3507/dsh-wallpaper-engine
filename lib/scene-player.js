...
(function() {
  ...
  const TEXTURE_CACHE_MAX = 64; // 修复: 纹理缓存原先无上限 (见 loadTexture)
  let maxVertexAttribs = 0; // 修复: MAX_VERTEX_ATTRIBS 原先每帧 gl.getParameter 一次
  let activeParticles = [];
  ...
  function renderFrame(now) {
    ...
    const _maxAttribs = maxVertexAttribs || (maxVertexAttribs = gl.getParameter(gl.MAX_VERTEX_ATTRIBS));
    for (let i = 0; i < _maxAttribs; i++) gl.disableVertexAttribArray(i);
  ...
  canvas.addEventListener('webglcontextlost', (event) => {
    ...
    contextLost = true;
    // 修复: 上下文丢失即释放缓存的纹理 (原先 textureCache 里的纹理永不 deleteTexture)
    for (const rec of textureCache.values()) {
      try { gl.deleteTexture(rec.texture); } catch (e) {}
    }
    textureCache.clear();
  });
  ...
  fetch('/wallpaper-engine/scene-manifest/' + token)
    ...
    .then(data => {
      if (data.ok && data.manifest) {
        ...
        try { window.parent.postMessage({ type: 'dsh-scene-unavailable' }, window.location.origin); } catch (e) {}
      }
    })
    .catch(err => {
