...
const selection = {
  ...
  // 仅 pkg 壁纸有）。存在且开关开启且无失败记忆时，scene 壁纸以 live iframe
  // 形态播放（buildMedia 最高优先级，见 sceneLiveEnabled）。
  sceneLiveSrc: null,
...
function applySelection(id) {
  ...
  if (w.type === "scene" && w.frameUrl && !selection.sceneVideo && !sceneLiveEnabled(selection)) queueSceneAnimUpgrade(w.frameUrl);
...
function sceneLiveEnabled(selLike) {
  return Boolean(selLike && selLike.type === "scene" && selLike.sceneLiveSrc
    && selLike.sceneLive !== false
    && !(selLike.sceneLiveFailures
      && selLike.sceneLiveFailures[String(selLike.id)] === true));
}
...
function sceneLiveUrl(selLike) {
  ...
  return "/wallpaper-engine/scene-live/index.html?type=scene&fit=" + fit
    + "&sceneFps=" + fps + "&muted=" + muted
    + "&src=" + encodeURIComponent(selLike.sceneLiveSrc)
...
function startLiveWatch(frame, wid) {
  ...
  watch.timer = setInterval(() => {
    ...
    if (watch.stall >= LIVE_STALL_TICKS * 2) liveFail();
  }, 1000);
  liveWatch = watch;
}
...
function buildMedia(sel) {
  // Scene 壁纸播放形态优先级:
  //   1. live — WebWallGL 实时 WebGL iframe（sceneLiveSrc 存在且无失败记忆；
  //      心跳判定失败后自动降级，见 startLiveWatch/liveFail）
  //   2. sceneVideo — 场景内嵌 MP4 (作者主分支, 硬件解码 <video>, poster=静态帧)
  //   3. scene-anim — beta 动画升级 (本分支 CPU 渲染视频, URL 含 /scene-anim/)
  //   4. 静态帧 img (frameUrl)
  // 未升级时仍是静态帧 img。
  const isSceneLive = sel.type === "scene" && sceneLiveEnabled(sel);
  const isSceneVideo = sel.type === "scene" && Boolean(sel.sceneVideo) && !isSceneLive;
  const isSceneAnim = sel.type === "scene" && sel.url && sel.url.indexOf("/scene-anim/") !== -1;
  const isStill = sel.type === "image" || (sel.type === "scene" && !isSceneLive && !isSceneVideo && !isSceneAnim);
  if (isSceneLive) {
    // 垫底静态帧（加载期/降级重建期画面连续）+ live iframe（首帧心跳通过后
    // 淡入）。返回数组形态与 Edge canvas 分支一致（syncLayers 逐个 append）。
    const poster = document.createElement("img");
    poster.src = sel.url; // frameUrl（含 ?v= 画面档位）
    poster.alt = "";
    poster.draggable = false;
    poster.className = "we-media we-live-poster";
    const frame = document.createElement("iframe");
    frame.src = sceneLiveUrl(sel);
    frame.setAttribute("frameborder", "0");
    frame.setAttribute("scrolling", "no");
    // iframe 内包内音频（HTMLAudioElement）的自动播放授权。
  ...
  if (selection.url) {
    const wantKey = selection.type + "\u0000" + selection.url + "\u0000"
      ...
      + "\u0000" + (selection.type === "scene"
        ? (sceneLiveEnabled(selection)
          ? "live\u0000" + selection.sceneLiveSrc + "\u0000" + selection.sceneLiveFps
          : "nolive")
        : "");
...
function WallpaperPicker(props) {
  ...
  const isLiveScene = sel.type === "scene" && sceneLiveEnabled(sel);
  ...
  function renderEffectsTab() {
    ...
    return React.createElement(React.Fragment, null,
      ...
      React.createElement("div", { className: "we-picker__section" },
        ...
        sel.type === "scene" && switchRow("场景实时渲染", sel.sceneLive !== false, (e) => {
          ...
          tooltip: "场景壁纸由 WebWallGL 实时渲染（粒子/脚本/视差/包内音频）；加载失败或运行失联时自动退回内嵌视频/静态帧。重新开启会重试此前失败的壁纸",
        }),
        sel.type === "scene" && sel.sceneLive !== false && sel.sceneLiveSrc
