...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    // 按壁纸的实时渲染失败记忆 { [wallpaperId]: 'timeout'|'stall'|'manual' }。
    // 由客户端写入：'timeout'/'stall' = 心跳判定失败（自动降级）；
    // **'manual' = 用户手动「改用静态帧」** —— 实时渲染"能出画面但看不过去"
    // （例如缺件/观感不对）时的人工回退，与自动失败共用同一条"按壁纸禁用实时渲染"的
    // 通道（liveRenderEnabled 只判真假），因此不需要第二套状态。重开 sceneLive 开关
    // 会清空整张表（= 全部重试）。必须进白名单, 否则 PUT 上来即被丢弃、刷新/重启后丢失
    // （踩坑回归：client 侧 sanitize 有、host 侧漏加）。
    sceneLiveFailures: (o.sceneLiveFailures && typeof o.sceneLiveFailures === 'object'
      ...
      ? Object.fromEntries(Object.entries(o.sceneLiveFailures)
          // 值是原因（客户端展示用）；兼容旧的 true。
          .filter(([k, v]) => typeof k === 'string' && k
            && (v === true || v === 'timeout' || v === 'stall' || v === 'manual')))
      : {},
...
export function apply(ctx) {
  ...
  //   variant       壁纸画面刷新档位 (上游 #91): 0 = 默认 (完整渲染 / maintexture 模式),
  //                 1 = 主纹理单张, 2 = 作者原画优先, 3 = 作者预览图, 4 = 自定义画面,
  //                 5 = 静态帧渲染（强制完整渲染）, 6 = 合成（分层）。
  //                 非 0 档进缓存键后缀 (见下), 各档互不覆盖; 0 档沿用旧键。
  async function ensureSceneFrame(abs, opts = {}) {
    const signal = opts.signal || null;
    const allowFallback = opts.allowFallback !== false;
    const vRaw = Number(opts.variant || 0);
    // 档 5 已退役（原「强制完整渲染」）：'改用静态帧' 现在是**按壁纸的显示形态开关**
    // （sceneLiveFailures[wid]='manual'），不在这条生成档位轴上。旧客户端若仍传 v=5，
    // 一律按 0（自动）处理，避免留下一个语义已废的 _v5 缓存键。
    const vParsed = Number.isFinite(vRaw) ? Math.floor(vRaw) : 0;
    const variant = (vParsed >= 1 && vParsed <= 6 && vParsed !== 5) ? vParsed : 0;
    ...
    if (!entry) {
      ...
      const pending = (async () => {
        ...
        // 档 1/2/6 (上游 #91 的「壁纸画面刷新」备选生成逻辑): 直接用提取逻辑产出,
        // **不经渲染 worker** —— 它们本就是比完整渲染更轻的生成方式。
        //   1 = 主纹理单张, 2 = 作者原画优先, 6 = 合成（分层, 即提取 variant 0）
        // 档 5「静态帧渲染」不在此列: 它要走下面的完整渲染。
        if (variant === 1 || variant === 2 || variant === 6) {
          const exVariant = variant === 6 ? 0 : variant;
          const frame = abs.toLowerCase().endsWith('.json')
            ? extractSceneMainImageFromDir(dirname(abs), basename(abs), exVariant)
            : extractSceneMainImage(new Uint8Array(await readFile(abs)), basename(abs), exVariant);
  ...
  function prewarmWanted() {
    try { return readSettings().scenePrewarm === true; } catch { return false; }
  }
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 壁纸画面刷新档位（?v=1..6，上游 #91 引入）：各档 = 不同**生成逻辑**，必须进缓存键；
      // 档 0/无参数沿用原键（既有帧零作废，见 ensureSceneFrame 的键兼容注释）。
      //   1 = 主纹理单张 · 2 = 作者原画 · 3 = 作者工坊预览图 · 4 = 自定义画面（截屏导入）
      //   6 = 合成（分层，强制走 pkg 提取的多层合成）
      //   5 = **已退役**：「改用静态帧」现在是按壁纸的**显示形态**开关
      //       （sceneLiveFailures[wid]='manual'），不属于生成档位轴；旧客户端若仍传 v=5
      //       一律按 0（自动）处理，避免留下语义已废的 _v5 缓存键。
