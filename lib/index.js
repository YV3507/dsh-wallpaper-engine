...
export function apply(ctx) {
  ...
  async function buildInventory() {
    ...
    const wallpapers = await Promise.all(all.map(async (w) => {
      ...
      return {
        ...
        // WebWallGL live render capability. Pkg-based scenes only: loose
        // scene.json dirs expose no scene.pkg for httpSource to fetch, so
        // those stay on the existing frame/video chain. The client assembles
        // the renderer URL itself: /scene-live/index.html?type=scene&src=
        // <sceneLiveSrc>&mediaBase=<origin>/wallpaper-engine/scene-files.
        sceneLive: w.type === 'scene' && hasFrame
          && !w.fileAbs.toLowerCase().endsWith('.json'),
        sceneLiveSrc: w.type === 'scene' && hasFrame
          && !w.fileAbs.toLowerCase().endsWith('.json') ? tokenFor(w.fileAbs) : null,
  ...
  // 3c-3. WebWallGL media endpoint: raw bytes straight from the wallpaper's
  //       own directory (scene.pkg / project.json / loose files). The renderer
  //       fetches {mediaBase}/{src}/scene.pkg etc. and parses the container
  //       itself — LZ4 / TEX / DXT decode all live in WebWallGL, the host never
  //       decodes anything on this path. Path-fenced to the wallpaper directory
  //       and Range-capable via serveFile (embedded audio/video seeking).
  disposers.push(webServer.register({
    kind: 'prefix',
    ...
    handler: (req, res) => {
      ...
      // project.json sits next to them. Fence the resolved target inside it.
