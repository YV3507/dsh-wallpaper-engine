...
const BUILTIN = new Set([
  ...
  'fract', 'mod', 'fmod', 'pow', 'exp', 'log', 'sqrt', 'inversesqrt', 'sign', 'sin',
...
class Transpiler {
    ...
    for (const node of this.ast.program || []) {
      if (node.type === 'declaration_statement') {
        ...
        const kind = quals.includes('uniform') ? 'uniform'
          ...
          // P1-8④: const 全局单独标记 — 初始化一次(程序级), 不进逐像素重置体
          : quals.includes('const') ? 'const'
          : 'global';
  ...
  generate() {
    ...
    for (const [name, info] of Object.entries(this.globals)) {
      if (info.kind === 'uniform') {
        ...
        // P1-8④: const 全局 — GLSL 语义不可写, 模块装配时初始化一次即可
        // (初始化行仍随每次 fragFn/vertFn 调用(每帧)执行, 引用 uniform 也语义不变)
        lines.push(`var ${name} = ${this.defaultValue(info.type)};`);
      } else {
        // 全局变量 (初始化器在下方 AST 扫描时补充)
        lines.push(`var ${name} = ${this.defaultValue(info.type)};`);
        initLines.push(this.resetStmt(name, info.type));
      }
    }
    ...
    for (const node of this.ast.program || []) {
      ...
      for (const d of dl.declarations || []) {
        ...
        if (d.initializer) {
          ...
          lines.push(line);
          // P1-8④: const 全局初始化一次 — 初始化行不进逐像素重置体
          if (this.globals[nm].kind !== 'const') initLines.push(line.replace(/^var /, ''));
        }
      }
    }
  ...
  // P1-8④: var 全局默认值逐像素重置语句 — vec/mat 免每像素重建 Float32Array:
  // 形状未变(长度一致)时原地清零, 否则(被 main 换成异长对象)回退重建; 数值语义等价
  resetStmt(name, type) {
    const n = VEC_SIZE[type] || MAT_SIZE[type];
    if (n) return `${name}.length === ${n} ? ${name}.fill(0) : (${name} = new Float32Array(${n}));`;
    return `${name} = ${this.defaultValue(type)};`;
  }

  // P1-8③: 取表达式第 i 分量码 — 结果带成分表 (comps, 见 postfixApply) 时直接选
  // 分量, 免运行期构造 [a,b,c] 数组再下标取回; 否则按旧路径下标访问
  compAt(e, i) {
    return e.comps ? e.comps[i] : `${e.code}[${i}]`;
  }
  ...
  declInit(alias, type, initializer, ctx) {
    ...
    if (dsz !== undefined) {
      ...
      if (esz !== undefined && esz !== dsz) {
        ...
        const parts = [];
        for (let i = 0; i < dsz; i++) parts.push(this.compAt(h, i));
        ...
        if (node.operator.literal === '-') {
          if (VEC_SIZE[inner.type]) {
            if (inner.simple) {
              ...
              const parts = [];
              for (let i = 0; i < n; i++) parts.push(`(-${this.compAt(inner, i)})`);
              return { code: `[${parts.join(', ')}]`, type: inner.type, simple: true, comps: parts };
            }
  ...
  binaryExpr(node, ctx) {
    ...
    if (op === '+' || op === '-' || op === '*' || op === '/') {
      // GLSL * 涉矩阵恒为线性代数乘 (vec*mat / mat*vec / mat*mat), 绝不逐分量 —
      // 官方 mul 宏 (#define mul(x,y) ((y)*(x)), ShaderUnit.cpp:30) 的展开体正是这个 *;
      // mul2(a,b) 按 GLSL b*a 求值 → 二元 l*r 以 mul2(r, l) 表达
      // (标量×mat 仍是 GLSL 逐分量广播, 走下方 _vmul 路径)
      if (op === '*' && (lmat || rmat) && (lvec || rvec || (lmat && rmat))) {
        return { code: `__rt.mul2(${r.code}, ${l.code})`, type: lvec ? l.type : r.type, simple: false };
      }
      if (lvec || rvec) {
        ...
        if (l.simple && r.simple) {
          ...
          const parts = [];
          for (let i = 0; i < n; i++) {
            // P1-8③: 操作数带成分表时直取分量, 免嵌套构造 [a,b,c] 再 [i] 取回
            const li = lvec ? this.compAt(l, i) : l.code;
            const ri = rvec ? this.compAt(r, i) : r.code;
            parts.push(`(${li} ${op} ${ri})`);
          }
          return { code: `[${parts.join(', ')}]`, type: lvec ? l.type : r.type, simple: true, comps: parts };
        }
  ...
  callExpr(node, ctx) {
    ...
    if (name) {
      ...
      if (VEC_SIZE[name]) {
        // P1-8③: vec 构造结果附成分表 (comps), 消费方按分量直取不再重建数组
        const v = this.constructVec(VEC_SIZE[name], args, ctx);
        return { code: v.code, type: name, simple: true, comps: v.comps };
      }
      if (MAT_SIZE[name]) {
      ...
      if (name === 'mix' && args.length === 3) {
        ...
        if (n && VEC_SIZE[b.type] && a.simple && b.simple && t.simple) {
          ...
          if (tVec || t.type === 'float') {
            const parts = [];
            for (let i = 0; i < n; i++) {
              const ti = tVec ? this.compAt(t, i) : t.code;
              parts.push(`(${this.compAt(a, i)} + (${this.compAt(b, i)} - ${this.compAt(a, i)}) * ${ti})`);
            }
            return { code: `[${parts.join(', ')}]`, type: a.type, simple: true, comps: parts };
          }
        }
      }
      // P1-8②: 高热内建内联 (dot/clamp/saturate/length/normalize) — 参数 simple 且
      // 类型可静态判定才内联, 否则保持 __rt.* 调用 (不确定安全性的一律不内联)
      const inl = this.inlineBuiltin(name, args, ctx);
      if (inl) return inl;
      if (BUILTIN.has(name) || CONSTS.has(name)) {
        const a = args.map((a) => this.expr(a, ctx).code).join(', ');
        // mul → mul2 通用分派: 官方宏 #define mul(x,y) ((y)*(x)) (ShaderUnit.cpp:30),
        // mul2(a,b) 按 GLSL b*a 求值 (HLSL 行主序 ↔ GLSL 列主序转置补偿);
        // 此前直连旧 runtime mul 仅覆盖 vec×mat 一种形态
        const rtName = name === 'mul' ? 'mul2' : name;
        return { code: `__rt.${rtName}(${a})`, type: this.builtinReturnType(name, args, ctx), simple: false };
      }
  ...
  builtinReturnType(name, args, ctx) {
    switch (name) {
      ...
      case 'mul': {
        // 官方 mul(x,y)→(y)*(x): mat×vec / vec×mat → vec (取 vec 侧类型), mat×mat → mat
        const t0 = args.length ? this.expr(args[0], ctx).type : null;
        const t1 = args.length > 1 ? this.expr(args[1], ctx).type : null;
        if (t0 && MAT_SIZE[t0] && t1 && VEC_SIZE[t1]) return t1;
        if (t0 && VEC_SIZE[t0] && t1 && MAT_SIZE[t1]) return t0;
        return t1 && MAT_SIZE[t0] && MAT_SIZE[t1] ? t1 : (t0 || 'vec4');
      }
  ...
  // P1-8②: 高热内建内联 — dot/clamp/saturate/length/normalize。
  // 安全边界: 参数 simple (无副作用、可重复求值) 且类型可静态判定才内联,
  // 其余 (类型未知/lo·hi 非标量/normalize 无 hoist 上下文) 返回 null 回退 __rt.*。
  // 数值序列与 runtime.js 实现逐位一致: dot/length 保留 `0 +` 首项与累加顺序
  // (-0 归约一致), clamp/saturate 保留 Math.min/max 实参与求值顺序。
  inlineBuiltin(name, args, ctx) {
    if (name === 'dot' && args.length === 2) {
      const a = this.expr(args[0], ctx), b = this.expr(args[1], ctx);
      const n = VEC_SIZE[a.type];
      if (n && VEC_SIZE[b.type] === n && a.simple && b.simple) {
        // runtime: s = 0; s += a[i]*b[i] — 左结合累加, 与下方串接等价
        let code = `0 + (${this.compAt(a, 0)} * ${this.compAt(b, 0)})`;
        for (let i = 1; i < n; i++) code += ` + (${this.compAt(a, i)} * ${this.compAt(b, i)})`;
        return { code: `(${code})`, type: 'float', simple: true };
      }
      return null;
    }
    if (name === 'length' && args.length === 1) {
      const x = this.expr(args[0], ctx);
      const n = VEC_SIZE[x.type];
      if (n && x.simple) {
        let code = `0 + (${this.compAt(x, 0)} * ${this.compAt(x, 0)})`;
        for (let i = 1; i < n; i++) code += ` + (${this.compAt(x, i)} * ${this.compAt(x, i)})`;
        return { code: `Math.sqrt(${code})`, type: 'float', simple: true };
      }
      return null;
    }
    if (name === 'normalize' && args.length === 1) {
      const x = this.expr(args[0], ctx);
      const n = VEC_SIZE[x.type];
      // 需 hoist 临时变量存长度 (各分量除法复用同一 l) — for 头部等无 hoist 上下文回退
      if (n && x.simple && ctx && ctx.hoist) {
        let s = `0 + (${this.compAt(x, 0)} * ${this.compAt(x, 0)})`;
        for (let i = 1; i < n; i++) s += ` + (${this.compAt(x, i)} * ${this.compAt(x, i)})`;
        const t = this.fresh();
        // runtime: const l = length(x) || 1; r[i] = x[i] / l
        ctx.hoist.push(`var ${t} = Math.sqrt(${s}) || 1;`);
        const parts = [];
        for (let i = 0; i < n; i++) parts.push(`${this.compAt(x, i)} / ${t}`);
        return { code: `[${parts.join(', ')}]`, type: x.type, simple: true, comps: parts };
      }
      return null;
    }
    if (name === 'clamp' && args.length === 3) {
      const x = this.expr(args[0], ctx), lo = this.expr(args[1], ctx), hi = this.expr(args[2], ctx);
      if (!x.simple || !lo.simple || !hi.simple) return null;
      if (!SCALARS.has(lo.type) || !SCALARS.has(hi.type)) return null; // 向量边界走 runtime 逐分量
      const n = VEC_SIZE[x.type];
      if (n) {
        const parts = [];
        for (let i = 0; i < n; i++) parts.push(`Math.min(${hi.code}, Math.max(${lo.code}, ${this.compAt(x, i)}))`);
        return { code: `[${parts.join(', ')}]`, type: x.type, simple: true, comps: parts };
      }
      if (SCALARS.has(x.type)) {
        return { code: `Math.min(${hi.code}, Math.max(${lo.code}, ${x.code}))`, type: x.type, simple: true };
      }
      return null;
    }
    if (name === 'saturate' && args.length === 1) {
      const x = this.expr(args[0], ctx);
      if (!x.simple) return null;
      const n = VEC_SIZE[x.type];
      if (n) {
        // runtime vec 路径 = clamp(x, 0, 1) → Math.min(1, Math.max(0, x[i]))
        const parts = [];
        for (let i = 0; i < n; i++) parts.push(`Math.min(1, Math.max(0, ${this.compAt(x, i)}))`);
        return { code: `[${parts.join(', ')}]`, type: x.type, simple: true, comps: parts };
      }
      if (SCALARS.has(x.type)) {
        return { code: `Math.min(1, Math.max(0, ${x.code}))`, type: x.type, simple: true };
      }
      return null;
    }
    return null;
  ...
  // 返回 { code, comps } (comps = 各分量码, P1-8③)
  constructVec(size, args, ctx) {
    if (args.length === 0) return { code: `new Float32Array(${size})`, comps: null };
    ...
    if (args.length === 1) {
      ...
      if (SCALARS.has(e.type)) {
        const parts = Array(size).fill(e.code);
        return { code: `[${parts.join(', ')}]`, comps: parts };
      }
      // vec 参数 → 截取/填充 (P1-8③: comps 直取分量)
      const parts = [];
      for (let i = 0; i < size; i++) parts.push(this.compAt(e, i));
      return { code: `[${parts.join(', ')}]`, comps: parts };
    }
    // 多参数: 拼接
    const parts = [];
    for (let e of exprs) {
      if (SCALARS.has(e.type)) parts.push(e.code);
        ...
        for (let i = 0; i < n; i++) parts.push(this.compAt(e, i));
      }
    }
    // 截取到 size
    const cut = parts.slice(0, size);
    return { code: `[${cut.join(', ')}]`, comps: cut };
  }
  ...
  constructMat(type, args, ctx) {
    ...
    for (const a of args) {
      ...
      if (vn) for (let i = 0; i < vn; i++) parts.push(this.compAt(e, i));
  ...
  postfixApply(base, pf, ctx) {
    ...
    if (pf.type === 'field_selection') {
      ...
      if (sel.length === 1 && SWZ[sel[0]] !== undefined) {
        ...
        return { code: this.compAt(base, SWZ[sel[0]]), type: t, simple: true };
      }
      if (sel.split('').every((c) => SWZ[c] !== undefined)) {
        ...
        // P1-8③: 成分表直取 (comps 在 simple 基座上随 h 保留), 免构造数组再取回
        const parts = idx.map((i) => this.compAt(h, i));
        const pre = base.type && base.type[0] === 'i' ? 'ivec' : base.type && base.type[0] === 'b' ? 'bvec' : 'vec';
        return { code: `[${parts.join(', ')}]`, type: pre + idx.length, simple: true, comps: parts };
      }
    }
    if (pf.type === 'quantifier' || pf.type === 'index') {
      // GLS-02: shaderfrog 7.0.1 下标节点是 quantifier (lb/expression/rb), 此前只查 'index' → v[0] 丢成 v
      const idxNode = pf.type === 'quantifier' ? pf.expression : pf.index;
      // P1-8③: 常量下标 + 基座已展开成分 (comps) → 转译期直接选分量,
      // 消掉运行期 [a,b,c][i] (建数组再取下标); 非常量下标保留旧路径
      if (base.comps && (idxNode.type === 'int_constant' || (idxNode.type === 'literal' && /^-?\d+$/.test(idxNode.literal || '')))) {
        const ci = Number(idxNode.type === 'int_constant' ? idxNode.token : idxNode.literal);
        if (Number.isInteger(ci) && ci >= 0 && ci < base.comps.length) {
          const bt = base.type || '';
          const elem0 = bt.endsWith('[]') ? bt.slice(0, -2) : bt[0] === 'i' ? 'int' : 'float';
          return { code: base.comps[ci], type: elem0, simple: true };
        }
      }
      const idx = this.expr(idxNode, ctx);
