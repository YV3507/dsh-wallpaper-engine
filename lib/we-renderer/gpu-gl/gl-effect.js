...
// sf41m 显存预算: 仅"个数 24"对 4K 纹理会到 ~800MB 且无约束 → 叠加总字节上限,
// 超限按 LRU 淘汰 (实测隔离测试: 多变尺寸负载曾把显存从 588MB 推到 1601MB)。
const TEX_CACHE_MAX_BYTES = 256 * 1024 * 1024;
let _texCache = new Map(); // texData → { tex, lastUse, bytes }
let _texCacheBytes = 0;
const texBytesOf = (t) => ((t && t.width ? t.width : 0) * (t && t.height ? t.height : 0) * 4);
...
function texCacheSet(gl, texData, tex) {
  const bytes = texBytesOf(texData);
  while (_texCache.size >= TEX_CACHE_MAX || (_texCacheBytes + bytes > TEX_CACHE_MAX_BYTES && _texCache.size > 0)) {
    if (!evictOldest(gl)) break;
  }
  _texCache.set(texData, { tex, lastUse: _texUseSeq++, bytes });
  _texCacheBytes += bytes;
  return tex;
}
...
// sf41m: 尺寸键数原本无上限 (每种新分辨率都建键) + 每键 8 个 → 多尺寸负载无界增长;
// 改为"总字节预算 + 跨尺寸 LRU 淘汰"(池内条目带 seq, 超预算时淘汰最旧)。
let _fboPool = new Map(); // "w x h" → [{fbo, tex, bytes, seq}, ...] 可用池
const FBO_POOL_MAX = 8; // 每种尺寸最多留 8 个 (效果链乒乓需要 2+)
const FBO_POOL_MAX_BYTES = 128 * 1024 * 1024;
let _fboPoolBytes = 0;
let _fboSeq = 0;
function freeFbo(gl, f) {
  if (!f) return;
  try { gl.deleteTexture(f.tex); } catch { /* ignore */ }
  try { gl.deleteFramebuffer(f.fbo); } catch { /* ignore */ }
}
function evictOldestFbo(gl) {
  let bestKey = null, bestIdx = -1, bestSeq = Infinity;
  for (const [k, pool] of _fboPool) {
    for (let i = 0; i < pool.length; i++) {
      if (pool[i].seq < bestSeq) { bestSeq = pool[i].seq; bestKey = k; bestIdx = i; }
    }
  }
  if (bestKey == null) return false;
  const pool = _fboPool.get(bestKey);
  const f = pool.splice(bestIdx, 1)[0];
  _fboPoolBytes -= (f.bytes || 0);
  if (!pool.length) _fboPool.delete(bestKey);
  freeFbo(gl, f);
  return true;
}
function acquireFbo(gl, width, height) {
  ...
  return { fbo, tex, key, bytes: width * height * 4, seq: _fboSeq++ };
}
function releaseFbo(gl, fbo) {
  if (!fbo) return;
  const pool = _fboPool.get(fbo.key);
  if (pool && pool.length >= FBO_POOL_MAX) { freeFbo(gl, fbo); return; }
  fbo.seq = _fboSeq++;
  if (!pool) _fboPool.set(fbo.key, [fbo]);
  else pool.push(fbo);
  _fboPoolBytes += (fbo.bytes || 0);
  while (_fboPoolBytes > FBO_POOL_MAX_BYTES) {
    if (!evictOldestFbo(gl)) break;
  }
}
/**
 * 轻量回收 (sf41m): 释放复用池与纹理缓存, 保留上下文/程序/白纹理。
 * 供宿主等长生命周期进程在渲染批次之间/闲置时调用 — 旧实现只有 worker 退出
 * 时的 disposeGPU(), 长驻进程的池与缓存永不回收。
 */
export function trimGPU() {
  try {
    if (_gl) {
      for (const [, e] of _texCache) { try { _gl.deleteTexture(e.tex); } catch { /* ignore */ } }
      for (const [, pool] of _fboPool) for (const f of pool) freeFbo(_gl, f);
    }
  } catch { /* ignore */ }
  _texCache.clear(); _texCacheBytes = 0;
  _fboPool.clear(); _fboPoolBytes = 0;
}
