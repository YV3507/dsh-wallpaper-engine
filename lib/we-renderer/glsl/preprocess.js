...
export function evaluateNumericIfs(source, defines) {
  ...
  const evalCond = (cond) => {
    // GLSL 预处理忽略注释: `#if TYPE == 0 // round` 的尾注会令判定正则失配
    ...
    for (;;) {
      ...
      for (let i = 0; i < c.length; i++) {
        if (c[i] === '(') depth++;
        else if (c[i] === ')') { depth--; if (depth === 0) { closed = i === c.length - 1; break; } }
      }
      if (closed) c = c.slice(1, -1).trim();
      else break;
    }
    // defined(NAME) / !defined(NAME)
    let dm = /^(!?)\s*defined\s*\(\s*([A-Za-z_]\w*)\s*\)$/.exec(c);
    if (dm) {
      const def = isMacroDefined(dm[2]);
      return dm[1] ? !def : def;
    }
    // 复合布尔: 拆最外层 && / || (|| 优先级更低, 先拆 — `A && B || C` = (A&&B)||C)
    const topLevel = (op) => {
      let depth = 0;
      for (let i = 0; i < c.length; i++) {
        if (c[i] === '(') depth++;
        else if (c[i] === ')') depth--;
        else if (depth === 0 && c.startsWith(op, i)) return i;
      }
      return -1;
    };
    const orIdx = topLevel('||');
    if (orIdx >= 0) {
      const a = evalCond(c.slice(0, orIdx));
      const b = evalCond(c.slice(orIdx + 2));
      if (a === null || b === null) return null;
      return a || b;
    }
    const andIdx = topLevel('&&');
    if (andIdx >= 0) {
      const a = evalCond(c.slice(0, andIdx));
      const b = evalCond(c.slice(andIdx + 2));
      if (a === null || b === null) return null;
      return a && b;
    }
  ...
  for (const l of lines) {
    ...
    if (!m) {
      ...
      if (m) {
        // C/GLSL 语义: #ifdef 未定义宏 = false, #ifndef = true — **可判定**。
        // 旧逻辑把未定义视为不可判定 → 回退扁平正则 → 嵌套 #if 留给 shaderfrog
        // → 解析崩 (frame_builder/rounded_mask 等 workshop shader 效果失效)。
        // 完整行级求值 (L232+) 用同一 isMacroDefined, 语义一致, 无需回退。
        continue;
      }
      continue;
    }
