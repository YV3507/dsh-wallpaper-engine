...
export function installText(proto) {
  Object.assign(proto, {
    ...
    // 避免缓存输出错误时间; "拆出文本层单独实时渲染" (渲染分离 + 客户端轮询) 未实施, 延后。
    ...
    renderTextObject(o, t) {
        ...
        // 例外: 若该对象被其它层以 _rt_imageLayerComposite_<id>_a 引用 (窗户玻璃上的
        // 时钟/日期合成), 则必须渲染并保留其图像, 否则下游图层永远取到空 RT
        // (实测 Angel Mail: RT 表 0 项, 窗户的 578/848 实例全部 miss)。
        if (this._isLiveText(o) && !(this._isRtReferenced && this._isRtReferenced(o.id))) return;
        ...
        if (fontPath) {
          try {
            ...
            // _rt_ 图层合成: 文本对象同样要进 RT 表 (供窗户玻璃等下游层采样)
