      ...
      if (msg && msg.ok) {
        ...
        const readFileBack = async () => {
          ...
          try {
            ...
            // APNG/PNG 临时文件永久留在 tmpdir (旧实现只在成功路径 unlink)。
...
// transcodeCacheDir 的 tc_*.mp4 每个 80-280MB, 场景帧缓存 sf34c_*.png/jpg 与
// san_*.apng|mp4、sv1_*.mp4 也从不淘汰 → 长期使用后无上限增长 (几十 GB)。
...
export function apply(ctx) {
  ...
  function absFromMediaUrl(u) {
    ...
    const m = /\/(?:scene-frame|scene-anim|media|scene-runtime|preview)\/([A-Za-z0-9_-]+)/.exec(u);
    if (!m) return null;
    ...
    handler: (req, res) => {
      ...
      if (!entry) {
        ...
        const pending = (async () => {
          ...
          // 进度清理 (内存项 + .prog 文件): 成功路径仍在原位置清 (保持进度条
          // 时序不变), 其余所有退出路径 (取消 / worker 拒绝 / 帧目录 mkdir
          // 失败) 由下方 finally 兜底 — 旧实现只在成功路径清, 早期 throw 会把
          // 进度内存项和 .prog 永久留下 (轮询一直读到旧条目)。
          let progCleaned = false;
          const cleanupProgress = async () => {
            if (progCleaned) return;
            progCleaned = true;
          ...
          if (isVideo) {
            ...
            // mkdir 失败在 try 之外, 进度清理要显式补一次 (否则内存项 + .prog 残留)。
            try { mkdirSync(framesDir, { recursive: true }); } catch (e) { gpuDiag('framesDir 创建失败:', e.message); await cleanupProgress(); throw new Error('frames dir create failed'); }
          }
          try {
            ...
            // 取消 / worker 拒绝等早期 throw 也要清进度 (幂等, 成功后无副作用)。
            await cleanupProgress();
