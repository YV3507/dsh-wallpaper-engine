window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = {
		  ...
		  sceneGpuAccel: false,
		  // 静态帧链**总开关**（父级是「场景实时渲染」）：实时渲染关掉后由它接管，默认开。
		  // 关掉 = 场景**不再渲染**静态帧：有内嵌 MP4 就放它，否则用作者预览图（若导入过
		  // 自定义画面则用它）。此时「出图来源」与「静态帧兜底与调优」整组都失去意义，界面一并隐藏。
		  sceneFrameRender: true,
		  // 空闲预热 (G1: 切换壁纸立即看到画面)：默认关闭 —— 它会在空闲时占一个渲染
		  // 进程并写帧缓存，必须由用户知情选择。范围 recent = 当前 + MRU 前 6 张；
		  // all = 全库场景（并发严格为 1，不会占满核心）。
		  scenePrewarm: false,
		  scenePrewarmScope: "recent",
		  // ── 有损路线总开关 ────────────────────────────────────────────────
		  // 集中管理"用观感换速度"的手段。默认关闭; 关闭时附属项一律失效。
		  // GPU 渲染加速 (sceneGpuAccel) 是**独立开关**, 不在其内 (并入会让默认配置掉性能)。
		  sceneLossyRoute: false,
		  // 画面来源模式 = 有损路线的附属项: "render" = 完整场景渲染; "maintexture" =
		  // 主纹理近似 (更快但画面不同: 不含角色骨骼合成/粒子/效果, 且含 puppet 的场景
		  // 会自动回退)。总开关打开时默认 maintexture, 关闭时强制 render。
		  sceneFrameSource: "render",
		...
		function sanitizeSettings(o) {
		  ...
		  return {
		    ...
		    // 有损路线总开关 (含旧配置迁移: 只设过 sceneFrameSource='maintexture' 的
		    // 用户自动打开总开关, 避免被静默切回完整渲染)。与宿主 sanitizeSettings 同式。
		    sceneLossyRoute: o.sceneLossyRoute === true
		      || (o.sceneLossyRoute === undefined && o.sceneFrameSource === "maintexture"),
		    sceneFrameSource: (o.sceneLossyRoute === true
		      || (o.sceneLossyRoute === undefined && o.sceneFrameSource === "maintexture"))
		      ? (o.sceneFrameSource === "render" ? "render" : "maintexture")
		      : "render",
		...
		// 推算「未导入自定义画面时的档位数」）。
		// short = 状态行里显示的短名（状态行栏位窄，见 frameVariantStatusText）；label =
		// 完整档名，留给状态行之外的解释性文字与注释，不进入 UI 的窄栏。
		const FRAME_VARIANTS = [
		  // 自动 = 走**自动回退链**：完整渲染 → 主纹理提取（合成优先）→ 作者预览图，逐级回退。
		  // 默认，也是唯一"会渲染"的一档。它不报档名，只报当前生效的来源（见下）。
		  { id: 0, label: "自动（逐级回退）" },
		  // 下面两档是**自动链上没有的替代来源**，只在自动链给出的画面也不满意时才需要手动指定：
		  { id: 1, label: "主纹理（单张大图，跳过合成）", short: "单张大图" },
		  { id: 2, label: "作者原画（内嵌 JPEG/PNG 优先）", short: "内嵌 JPEG/PNG" },
		  // 自定义画面（用户截屏）——最可靠的逃生口，作为最后一档。
		  { id: 4, label: "自定义画面", short: "自定义画面" },
		];
		...
		// 主纹理近似，见 sceneFrameSource）；直接指定的档则报该档短名。
		function frameVariantStatusText(selLike) {
		  ...
		  const short = idx === 0
		    ? (selLike.sceneFrameSource === "maintexture" ? "主纹理近似" : "完整渲染")
		    : (FRAME_VARIANTS[idx].short || FRAME_VARIANTS[idx].label);
		  return (idx + 1) + "/" + total + " 档  当前：" + short;
		}
		...
		/**
		 * `?cached=1` 变体：宿主**只查缓存、绝不渲染**（未命中回 404）。
		 * 用途是 live 的垫底图 —— 静态帧是"缓存兜底"，不该为一张过渡图触发 4K 冷渲染
		 * 去抢实时渲染的 CPU/GPU（见 buildMedia 的 isLive 分支）。
		 */
		function cachedOnlyFrameUrl(frameUrl) {
		  if (!frameUrl) return null;
		  return frameUrl + (frameUrl.indexOf("?") === -1 ? "?" : "&") + "cached=1";
		}
		...
		function applySelection(id) {
		  ...
		  // Web 壁纸是**多文件 HTML 应用**（官方 CORSAIR Collection / Corsair-O-Tron 都是
		  // Angular/bundler 产物），入口里的资源全是相对路径。iframe 的 src 决定相对解析
		  // 基准：`/media/<入口 token>` 没有尾斜杠 ⇒ 基准是 `/wallpaper-engine/media/`
		  // ⇒ 每个子资源都变成 `/wallpaper-engine/media/<文件名>`，而 /media 只按单文件
		  ...
		  const frameRenderOff = w.type === "scene" && selection.sceneFrameRender === false;
		  const hasCustom = Boolean((selection.customFrames || {})[String(w.id)] === true || w.hasCustomFrame);
		  const variantForUrl = frameRenderOff ? (hasCustom ? CUSTOM_FRAME_ID : 3) : savedVariant;
		  selection.url = w.type === "scene" ? frameUrlWithVariant(w.frameUrl, variantForUrl)
		...
		function buildMedia(sel) {
		  ...
		  if (isLive) {
		    // 显示优先级：**实时渲染优先，静态帧降级为"缓存兜底"**（它的本意就是场景类壁纸
		    // 出不了好效果时的取舍方案）。
		    // 垫底图因此只在**静态帧已经躺在缓存里**时才摆（`?cached=1`：宿主只命中、绝不渲染）。
		    // 未命中就空着等实时渲染出画面 —— 宁可牺牲首次加载，也不要为一张过渡图跑 4K 冷渲染
		    // （worker + GPU，数百 ms~数秒）去和实时渲染的首帧抢 CPU/GPU：实测后果是卡顿、首帧
		    // 超时判失败（liveFail 写进失败记忆，此后该壁纸一直不实时渲染）与黑屏。
		    // 作者预览图**不再**当垫底（画质太差，用户明确否掉）；网页壁纸没有静态帧，维持预览图。
		    ...
		    const posterSrc = sel.type === "scene"
		      // 场景：只认缓存命中；「静态帧渲染」总开关关掉时连缓存也不摆（用户明确不要它）。
		      ? (sel.sceneFrameRender === false ? null
		        : cachedOnlyFrameUrl(sel.url && sel.url.indexOf("/scene-frame/") !== -1 ? sel.url : sel.sceneFrameUrl))
		      : (sel.type === "web" ? (sel.previewUrl || null) : null);
		    ...
		    // 未命中（宿主回 404）⇒ 摘掉垫底图，留空等实时渲染（首次加载为空是接受的取舍）。
		    poster.addEventListener("error", () => { try { poster.remove(); } catch { /* ignore */ } });
		    return [poster, frame];
		  }
		  ...
		  if (sel.type === "video") {
		    ...
		    media.src = sel.url || sel.previewUrl;
		    media.alt = "";
		    ...
		    // 应用型壁纸没有帧 URL, 直接就是预览图 (上面的 src 已兜底)。
		    if (sel.previewUrl) {
		      media.onerror = () => {
		        ...
		        React.createElement("div", { className: "we-picker__current" },
		          ...
		          React.createElement("div", { className: "we-picker__current-info" },
		            ...
		            React.createElement("div", { className: "we-picker__current-meta" },
		              current
		                ? ({ video: "视频壁纸", web: "网页壁纸", image: "图片壁纸", scene: isLiveScene ? "场景壁纸（实时渲染）" : "场景壁纸（静态帧）", application: "应用型壁纸（本插件不支持渲染）" }[current.type] || "壁纸") + (playbackLive ? " · 播放中" : " · 已暂停")
		                : "尚未选择壁纸"),
		            // 应用型壁纸（WE application，入口 .exe，如官方默认壁纸 Sheep）: 插件无法
		            // 内嵌运行 —— 只服务预览图。过去它被判成 scene，界面既不说明、后台还反复
		            // 排"渲染"任务并失败回退；这里明确告知，避免被当成"渲染坏了"。
		            current && current.type === "application"
		              && React.createElement("div", { className: "we-picker__current-error" },
		                "这是应用型壁纸（.exe）：Wallpaper Engine 的应用无法在浏览器内运行，这里只显示作者预览图。"),
		  ...
		  function renderEffectsTab() {
		    ...
		    return React.createElement(React.Fragment, null,
		      ...
		      React.createElement("div", { className: "we-picker__section" },
		        ...
		        //
		        // 组标题**只占一行**：原先「静态帧兜底（回退）」标题下面还有一行「调优项」，但那
		        // 行本身不是开关、也不承载任何值，纯粹给下面的行加语义前缀 —— 现已并进标题行
		        // 右侧的说明（见 .we-picker__section-head 的两个子元素）。
		        //
		        // 标题写「兜底**与**调优」是刻意的：本组横跨**两条不同的轴** ——
		        //   · 来源 / 回退轴：**出图来源**（画面从哪一个生成器来：自动链 / 主纹理单张 /
		        //     作者原画 / 自定义画面）。它不拿观感换速度，故**不属于调优**；它是自动链的
		        //     手动覆盖，语义上归"回退"。
		        //   · 调优轴：有损路线（用观感换速度）、空闲预热（何时算）、GPU 渲染加速（用什么
		        //     执行效果链）。这三项才是"调优"。
		        // 叫「调优项」会把出图来源错误地暗示成"会更慢/更快"，所以标题必须同时覆盖两轴。
		        // 行序也按这条轴来排：**来源在前（第一行）、调优在后** —— 画面不对时第一反应是
		        // 换来源，而不是去调速度。
		        (sel.type === "scene" || sel.type === "web") && !liveRenderEnabled(sel)
		          && sel.sceneFrameRender !== false
		          && React.createElement("div", { className: "we-picker__section" },
		          React.createElement("div", { className: "we-picker__section-head" },
		            React.createElement("span", { className: "we-picker__section-label" }, "静态帧兜底与调优"),
		            React.createElement("span", { className: "we-picker__hint" }, "只作用于上面的静态帧渲染"),
		          ),
		          // ── 出图来源：**自动回退链**的手动指定（来源轴，故排第一行）────────
		          // 它是"兜底"，所以只列自动链**没有**用到的替代来源（"合成 / 预览图"正是自动链
		          // 自己的第 2/3 步，摆出来等于让用户重试刚失败的那一步 —— 见 FRAME_VARIANTS 注释）。
		          // ⚠️ 下一条行必须保持"本组第一个控件"：本行之前不得插入任何 switchRow
		          // （护栏 R38 的 srcFirst 断言断死了这一点 —— 「来源在前、调优在后」是本组的语义）。
		...
		function UpdateNotice() {
		  ...
		  return React.createElement("div", { className: "we-update-notice", role: "alert" },
		    React.createElement("div", { className: "we-update-notice__title" }, "🎉 v0.7.5 更新（含 0.7.4 全部内容）：字体重做 · 场景壁纸修复 · 音频控制"),
		    React.createElement("div", { className: "we-update-notice__body" },
		      React.createElement("p", null, "自 0.7.3 以来的全部更新："),
		      ...
		      React.createElement("p", null,
		        "⑤ ", React.createElement("strong", null, "壁纸透明度不再破坏玻璃模糊（v0.7.5）"),
		        "：修复了调壁纸透明度后玻璃模糊失效、玻璃后面文字透出的问题（渲染引擎对透明背景放弃模糊，已通过壁纸层垫主题实色绕开，淡出语义不变）。"),
		      React.createElement("p", null,
		        "⑥ ", React.createElement("strong", null, "视频壁纸音轨控制（社区 PR #99）"),
		        "：视频壁纸可调音量 + 卡片一键「音乐开/关」；默认仍为静音。"),
		      React.createElement("p", null,
		...
		const CSS = `
		  /* Wallpaper layer: a fixed child of <body>, sunk BELOW the app frame.
		     壁纸透明度（#82）作用在**媒体叶子**（.we-layer .we-media）而非整层：layer 自身
		     保持不透明并垫一块主题实色（--we-wallpaper-fade-bg）—— 透明的 backdrop 会让
		     backdrop-filter（玻璃模糊）静默失效。对 <video>/<img>/<iframe>/canvas 四类媒体
		     统一生效，也无需逐媒体处理 fit/transform 的相互作用；变量缺省 1。 */
		  ...
		  /* ── 文字面可读性下限 (text-surface readability floor, #82) ───────────────
		     ...
		     --we-wallpaper-opacity 不参与本层：壁纸透明度只作用于壁纸层（v0.7.5 起具体落在
		     媒体叶子上，见 .we-layer / .we-layer .we-media 的注释）。 */
		  body {
		  ...
		  /* 标题行可容纳第二个子元素（如「静态帧兜底与调优」+「只作用于上面的静态帧渲染」），
		     故留出间距；其余分组只有一个子元素，gap 无副作用。 */
		  .we-picker__section-head { display: flex; align-items: center; gap: 8px; }
