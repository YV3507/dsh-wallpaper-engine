...
- **玻璃调节统一**（v0.3.3–v0.3.5）：设置窗口的玻璃模糊与**对话栏共用同一套调节参数**（「雾化」滑动条 0–60 px 同时控制设置窗口与输入栏/气泡的模糊半径，饱和度/亮度/对比度配方一致）；新增「**玻璃颜色**」—— 设置窗口玻璃的**底色色调**可自定义（6 预设 + 自定义取色，默认浅色白 / 深色深夜蓝，选定后两种主题统一使用该色），与「配色」（交互元素）分工：**配色管控件、玻璃颜色管玻璃本身**。
...
- **遮挡暂停（省电三档，「高级」页签）**：类似 Wallpaper Engine 的「被遮挡时暂停」——最小化 / 切页、窗口失焦、使用电池供电时自动暂停视频壁纸，**解码引擎直接归零**；回到界面 / 接通电源自动继续（网页壁纸仅随页面隐藏被浏览器节流）。三档开关均持久保存。
...
- **字体自定义**（v0.6.7）：「外观」页签新增「全局字体」分区——总开关默认关闭（即 dsh 原生外观），开启后可调 **字体颜色 / 字重(100–900) / 字体族**（默认 · 雅黑 · 楷体 · 宋体 · 黑体 · 行楷 · 等宽，选项按钮以各自字体实时预览）；报错红字不受染色影响，关闭总开关即一键恢复默认。
- **切换过场**：换壁纸（手动点选与自动轮播共用）可选的转场动画 —— **硬切**（默认，零成本即时切换）/ **交叉淡化** / **推移** / **线性擦除** / **圆形光圈** / **缩放溶解** / **条带**（锯齿扫过）；每种过场自带合适基准时长，再乘「快 / 标准 / 慢」三档，方向型过场另有方向可选。所有过场只动 `transform` / `opacity` / `clip-path`（合成器友好：满屏视频与 live 渲染页都不掉帧），且**旧画面始终保持不透明垫在新画面之下** —— 这是玻璃模糊（`backdrop-filter`）持续生效的前提。系统开启「减少动态效果」时一律按硬切处理。详见[切换过场](#切换过场转场动画)
...
- **输入光标颜色**（[#83](https://github.com/elysia395/dsh-wallpaper-engine/issues/83)）：「外观」页签新增 **输入光标** 分区——光标颜色与壁纸相近看不清时，可从 6 种预设或自定义取色器里挑一个高对比颜色（也可选「自动」恢复 dsh 原生表现）；作用于所有输入框与可编辑区域，独立于字体自定义开关。
...
**空帧门禁与清除通道**：体积阈值不可靠（headless 实测全黑 PNG：960×540≈12KB、1080p≈44KB、4K≈165KB，都远超固定字节闸），因此抓帧前会把 canvas 降采样到 64×64 看亮度分布 —— 近全黑或几乎无对比度判为「还没渲染出画面」，直接放弃回填（保留 CPU 帧），另加分辨率相关的体积地板（≈0.02 B/px）。抓到不满意的一帧时，在**设置 → 效果 → 画面 → 「实时帧」**里有三个入口：**微缩预览**（就是切换途中 / live 首帧前显示的那张静帧，与层里正在用的 URL 同源 —— 附像素尺寸）、**「重新截」**（按**当前**画面重抓一张并替换缓存；抓不到就原样保留，原因显示在行内）、**「清除 GPU 帧」**（等价于 `DELETE /wallpaper-engine/scene-frame-cache/<token>`，或 `POST …?clear=1` —— 删后 HEAD 回到 `X-WE-GPU: 0`、当前档位立即生效、下次 live 会重新抓取）。

> **这一行不受「场景实时渲染」开关影响**：那张实时帧正是切换壁纸途中与 live 首帧之前屏幕上显示的画面，构图不对（黑帧 / 旧视口 / 切走瞬间抓的）时用户必须能**立刻重抓**，而不是先关掉实时渲染再回来。同理**「自定义画面」也一直显示**（导入截图与实时渲染互不干扰）。只有**「壁纸画面刷新」**（换 CPU 生成的静态帧档位）在实时渲染生效时隐藏 —— 那时换档位不会生效，摆出来只会误导。预览图只在槽里**确实有**实时帧时出现（面板用 HEAD 探测，30s 去重；`X-WE-GPU: 1`），否则会显示成 CPU 档位帧、反而误导。
...
页签指示胶囊随选中项平滑滑动；设置页与壁纸仓库抽屉**共用同一个已保存页签**（存在浏览器 `localStorage`，不进配置文件）——在任一处切换，下次打开两边都按这个页签恢复。长说明一律收进控件悬停提示（tooltip），行内只保留一句话简述。
...
> **设置页签一览**：「壁纸」（选择 / 切换过场 / 轮播 / 上传）·「外观」（配色、玻璃颜色 / 透明度、**细节**：雾化 + 边框、**全局字体**、输入光标、窗口与侧栏）·「吉祥物」·「效果」（画面滤镜与透明度、实时渲染、播放与适配）·「声音」（音轨、系统音频、媒体信息 / 歌词）·「高级」（浏览方式、兼容性、省电、实时渲染诊断）。

### 切换过场（转场动画）

换壁纸时用哪种转场。**手动点选与自动轮播共用同一套设置**（「壁纸」页签 → 「切换过场」→ 「过场动画」**下拉菜单** —— 过场会持续增加，下拉天然可扩展，不用重排面板）。默认是**硬切** —— 先上零成本、零风险的即时切换，等把「最帅的」定下来再改默认（改 `DEFAULTS.switchTransition` 一处即可）。

| 过场 | 观感 | 基准时长 |
|---|---|---|
| **硬切**（默认） | 不动画，即时切换 | — |
| **交叉淡化** | 新画面淡入盖住旧画面 | 1800 ms |
| **推移** | 新画面从一侧推入，旧画面被同时推出 | 700 ms |
| **擦除** | 硬边从一侧扫过揭示新画面 | 700 ms |
| **光圈** | 圆形从画面中心张开 | 800 ms |
| **缩放** | 新画面轻微缩小淡入，旧画面同时微微前推 | 900 ms |
| **条带**（百叶窗） | 7 块板（横向过场是横板、纵向是竖板）从进入侧一起张开，板间留缝、末段缝隙闭合 | 800 ms |

**方向**只对方向型过场（推移 / 擦除 / 条带）有意义 —— 推移 / 擦除决定从哪一侧进入，条带决定竖条还是横条。**时长**是每类型基准 × 「快（0.6×）/ 标准（1×）/ 慢（1.6×）」，控件上直接显示折算后的毫秒数。

实现上有三条硬约束，也是这些过场能被选中的原因：

1. **只动 `transform` / `opacity` / `clip-path`** —— 全是合成器友好属性。本插件在 `<video>` 与 live 渲染 `<iframe>` 上踩过 `mask` / `filter` 掉出合成层的坑，所以一律不用（条带因此走 `clip-path` 多边形：N 块板彼此不相连，靠一条贴在进入侧边缘、宽度随进度从 0 长起的「脊」串成**单一**连通多边形）。
2. **旧画面始终保持不透明，垫在新画面之下** —— 玻璃面板的 `backdrop-filter` 一旦采样到透明背景就会静默失效（`--we-wallpaper-fade-bg` 就是为这个加的）。所以这里没有任何「淡到黑再淡出」这种中间态透明的方案，也没有引入第三层。
3. **单一真源**：过场时长由内联 `--we-switch-ms` 驱动（CSS 三属性都读它），「交叉淡化」的基准直接引用 `ROTATION_FADE_MS` 常量 —— 不会再出现「改了常量忘了改 CSS」的漂移。

系统开启「减少动态效果」（`prefers-reduced-motion`）时，所有过场一律按硬切处理。过场结束会把新层的临时内联样式清干净（否则满屏视频会永久占一个合成层）。

### 遮挡暂停（省电三档）

类似 Wallpaper Engine 的「被遮挡时暂停」——桌面端大部分时间 GPU≈0 的主因。浏览器无法直接探测"被窗口遮挡"，插件用三个最接近的信号（「高级」页签开关，即时生效、持久保存）：
...
> 开启「设置窗口液态玻璃」后，**General、模型、插件等所有原生分区**和左侧导航都会变成同一套液态玻璃 + 配色（通过覆盖设置对话框作用域内的 shell token 实现，不侵入其他界面）。设置窗口的玻璃模糊与**对话栏使用同一套调节参数**：「雾化」滑动条（0–60 px）同时控制设置窗口与输入栏/气泡的模糊半径，饱和度/亮度/对比度配方完全一致；**玻璃颜色**决定玻璃底色本身的色调（默认浅色白/深色深夜蓝，选定后两种主题统一使用该色），**玻璃透明度**决定浓淡，越高越"透"（壁纸颜色更清晰地透过面板），越低越接近实色。不支持 `backdrop-filter` 的浏览器自动回退到高不透明实色，保证文字可读。所有控件即时生效并保存在浏览器 `localStorage`，刷新不丢。

> **文字面可读性下限（默认开启，本版不提供开关）** —— 每个**承载文字的面**（输入栏卡片与工具弹窗、消息气泡、设置窗口的三层底板、侧栏面板与编辑器/终端内容面、插件自己的壁纸仓库抽屉与面板弹窗）都在玻璃色之下再压一层**主题底色**：浅色主题为白，深色主题为深夜蓝，固定权重 45%（深色主题 59%）。数值是按 **WCAG 正文 4.5:1** 在「玻璃透明度 {0,15,30,45,60} × 主题 × 壁纸透明度 {0,50,90}」全网格上实测出的最小值（最坏情况取浅色主题遇全黑壁纸像素、深色主题遇全白壁纸像素）：修复前后正文对比度从最坏 **1.00:1 提升到 4.63:1**。因此**把「玻璃透明度」拉到最高档、或把「壁纸透明度」调高，都不会再让文字看不清** —— 这两个滑条只作用于玻璃色那一半权重，下限那一半固定不动。语义不变：玻璃透明度依旧是"越大越透"且单调生效（仍可把面板调得更透/更实），只是压不下去下限；壁纸透明度依旧只作用于壁纸层（`.we-layer` 整层保持不透明、淡出落在媒体叶子上），仍是"融向页面底色"，与下限互不影响。
...
「外观」页签提供独立的字体自定义分区（分组名「全局字体」）。**总开关默认关闭**——此时整个界面维持 dsh 原生字体外观，不注入任何样式；开启后才应用下方三项，各项改动即时生效并持久保存（调节面板自身的标签文字始终保持主题墨色、不跟随「字体颜色」染色，保证可读性）：
...
输入框的光标颜色由 dsh 主题决定，壁纸透过液态玻璃输入框直接衬在光标后面——两者颜色相近时光标就「隐形」了（[#83](https://github.com/elysia395/dsh-wallpaper-engine/issues/83)）。「外观」页签的 **输入光标** 分区给光标一个独立的颜色项：
...
「效果」页签（壁纸激活后）提供六个滑动条，微调壁纸与界面的融合效果（**边框** 与 **雾化** 是全局材质细调，在「外观」页签的「细节」段）：
...
| **壁纸透明度** | 壁纸层的透明度（越大越透）：淡出后壁纸融向**原生底色**（**浅色纯白、深色纯黑**），即 IDEA 背景图式的「看得见但不喧宾夺主」。与**暗化**互补——一个减淡壁纸本身、一个压暗整体画面；想要融底色效果，建议「透明度调高 + 暗化调低」组合使用。场景壁纸的垫底静态帧会在实时画面出场后退场，不会在淡出时透出来 | 0–90 % | 0 % |
...
「声音」页签里有两项与系统声音有关的开关（都默认开启），外加一项联网开关（默认关闭）：
...
> 侧边栏液态玻璃适配与「外观」页签的字体自定义（行楷）同时生效的实际观感。
