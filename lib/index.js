...
export function apply(ctx) {
  ...
  async function buildInventory() {
    ...
    const wallpapers = await Promise.all(all.map(async (w) => {
      ...
      return {
        ...
        // 仅当 svKnown === true (缓存确认内嵌 MP4) 才输出 URL。
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    const key = PIPELINE_VERSION + '_' + gpuFlag + srcSuffix + '_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
        ...
        if (srcSuffix === 'm') {
          try {
            mainTexFrame = abs.toLowerCase().endsWith('.json')
              ? extractSceneMainImageFromDir(dirname(abs), basename(abs))
              : extractSceneMainImage(new Uint8Array(await readFile(abs)), basename(abs));
          } catch (e) {
            mainTexFrame = null;
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        const { fileAbs } = await ensureSceneFrame(abs, { signal: ctrl.signal });
        res.setHeader('Content-Type', fileAbs.endsWith('.jpg') ? 'image/jpeg' : 'image/png');
        res.setHeader('Cache-Control', 'no-store');
