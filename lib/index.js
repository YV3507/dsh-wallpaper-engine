...
export function apply(ctx) {
  ...
  async function refreshPrewarmCandidates(currentAbs) {
    ...
    let list = [];
    if (currentAbs) list.push(currentAbs);
  ...
  function prewarmWanted() {
    try {
      ...
      if (!(st.scenePrewarm === true && st.sceneFrameRender !== false)) return false;
      // 实时渲染在用 ⇒ 静态帧没有显示用途，而且预热是**满载 CPU/GPU** 的冷渲染，
      // 会与动画渲染抢资源（用户报告的黑屏/卡顿根因之一）。判据与客户端
      // liveRenderEnabled 同口径，见 liveSuppressesPrewarm。
      if (liveSuppressesPrewarm(st, absFromMediaUrl(st.url))) return false;
      return true;
    } catch { return false; }
  }
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        const { fileAbs } = await ensureSceneFrame(abs, { signal: ctrl.signal, variant });
