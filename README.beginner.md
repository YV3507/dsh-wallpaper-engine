...
- 装好之后，打开 `dsh web`，你的聊天界面后面就会显示一张动态壁纸（视频 / 网页 / **场景实时渲染**）。
...
| 想做什么 | 在哪里点 | 备注 |
|---|---|---|
| **换壁纸** | 选择壁纸 → 弹窗里点一张 | Video / Web / Scene / 上传的图视频都行 |
...
| **镜像翻转** | 「效果」页签 → 水平翻转 | 所有壁纸类型都行（含场景） |
| **场景壁纸不动 / 想换渲染方式** | 「效果」页签 → 场景实时渲染（默认已开） | 关掉它才会出现静态帧链；再往下是「出图来源」 |
...
**3. 场景（Scene）壁纸为什么不动 / 是静止的图片？**
- **正常情况下场景壁纸是会动的**：插件内置 **WebWallGL 实时渲染引擎**，在网页里把整个场景实时画出来（人物、水、粒子、特效、鼠标视差、包内音乐都会动）。这个开关在「效果」页签，叫「**场景实时渲染**」，**默认就是开着的**。
- 如果它不动，按顺序看三件事：①「场景实时渲染」是不是被关掉了（关掉后由「静态帧渲染」接管，出的是一张**完整场景画面**，不再动）；②开关下面有没有写失败原因（「首帧超时」/「运行中断」）—— 有就重新打开这个开关，它会清空失败记忆并重试；③这张壁纸是**松散 `scene.json` 目录**（不是 `.pkg`）或你的浏览器不支持 WebGL2 —— 这两种情况**只能**出静态画面，属于限制而不是 bug。
- 想换一种静态出图方式（或渲染器生成不了这张场景），在「效果」页签往下找「**出图来源**」，点「换一种」轮换；最可靠的办法是「自定义画面」：手动对电脑桌面截图后导入（画质 = 截图分辨率）。
...
| **Scene（场景）壁纸** | Wallpaper Engine 用 3D 引擎实时渲染的壁纸；插件用内置 **WebWallGL 引擎**在网页里**实时渲染**它（会动），渲染不可用时回退成一张静态画面。 |
| **静态帧** | 场景壁纸渲染出来的**一张完整画面**（不再动），实时渲染不可用时由它接管；「出图来源」可以换生成方式。 |
