...
export function installPuppet(proto) {
  Object.assign(proto, {
    renderPuppet(o, model, tr, t) {
        ...
        const sizeNorm = (() => {
          const force = process.env.DSH_WE_PUPPET_SIZE; // '1' 强制开 / '0' 强制关 (A/B 用)
          if (force === '0') return null;
          // 声明式开关 (关键): 模型 JSON 的 autosize —— 皓风琦 N/nv/十字架、Plana 人物/后发、
          // Mutsumi 嘴巴/眉毛 等**全部** `"autosize": true`, 即"把模型铺满声明 size"由
          // 壁纸文件自身声明, 而非全局行为/环境变量。
          const autoOn = !!(model && model.autosize === true);
          if (!autoOn && force !== '1') return null;
        ...
        for (const layer of layers) {
          ...
          for (let b = 0; b < nb; b++) {
            if (layer.additive) {
              ...
              // 比值语义: final.sx *= (lw.sx/ref.sx)^blend — 帧0 ref = bind (sx=sy=1)
              // 时退化为 lw.sx^blend; blend=1 时 = lw.sx 全量。
              const ref = refRT[b];
              ...
              if (ref.sx > 1e-9 && ref.sy > 1e-9 && lw[b].sx > 0 && lw[b].sy > 0) {
...
,
    _sampleAnimRT(mesh, anim, frame, nb, bones) {
        ...
        for (let b = 0; b < nb; b++) {
          ...
          const f = totalFrames > 1 ? (frame % totalFrames) : 0;
          ...
          if (mdla >= 0) {
            try {
              ...
              for (let a = 0; a < animCount && p + 12 < buf.length; a++) {
                ...
                animations.push({ id: animId, name: animName, loop: loopMode, fps: animFps, frameCount, boneCount, segBytes, segs, segPer, segFlags });
