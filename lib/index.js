...
export function apply(ctx) {
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    const variant = Number.isFinite(vRaw) && vRaw >= 1 && vRaw <= 6 ? Math.floor(vRaw) : 0;
    ...
    // 档 5 = 显式「静态帧渲染」: **强制完整渲染**, 忽略有损路线的 maintexture 近似。
    // 因为产物就是默认渲染产物, 键也与默认渲染档共用 (srcSuffix='' 且不加档位后缀)
    // —— 同一份帧不会被复制成两份缓存。
    const forceRender = variant === 5;
    if (forceRender) srcSuffix = '';
    // 画面刷新档位后缀: **仅非 0 档追加** —— 0 档必须沿用旧键 (既有缓存零作废, 见上),
    // 各档之间也互不命中。档 5 (强渲染) 例外: 它与默认渲染档同产物同键。
    const vSuffix = (variant && !forceRender) ? '_v' + variant : '';
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 壁纸画面刷新档位（?v=1..6，上游 #91 引入，5/6 为本仓库新增）：各档 = 不同生成
      // 逻辑，必须进缓存键；档 0/无参数沿用原键（既有帧零作废，见 ensureSceneFrame 的键兼容注释）。
      //   1 = 主纹理单张 · 2 = 作者原画 · 3 = 作者工坊预览图 · 4 = 自定义画面（截屏导入）
      //   5 = 静态帧渲染（强制完整渲染，忽略有损路线的近似模式；与默认渲染档共用缓存）
