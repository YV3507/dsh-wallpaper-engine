...
export function apply(ctx) {
  ...
  function sceneFieldsFor(w, hasFrame, customIds) {
    ...
    return {
      ...
      // 「壁纸属性」面板的数据入口（项目目录里的 project.json general.properties）
      propsUrl: isScene ? `${BASE}/props/${tokenFor(w.fileAbs)}` : null,
  ...
  function webFieldsFor(w, entryOk, mediaBase) {
    ...
    return {
      ...
      webLiveSrc: isWeb
        ...
        : null,
      propsUrl: isWeb ? `${BASE}/props/${tokenFor(w.fileAbs)}` : null,
    };
  }
