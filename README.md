...
| **条带** | 锯齿条纹扫过（齿在中段最宽、两端收拢成一条边） | 800 ms |

**方向**只对方向型过场（推移 / 擦除 / 条带）有意义 —— 推移 / 擦除决定从哪一侧进入，条带决定竖条还是横条。**时长**是每类型基准 × 「快（0.6×）/ 标准（1×）/ 慢（1.6×）」，控件上直接显示折算后的毫秒数。

实现上有三条硬约束，也是这些过场能被选中的原因：

1. **只动 `transform` / `opacity` / `clip-path`** —— 全是合成器友好属性。本插件在 `<video>` 与 live 渲染 `<iframe>` 上踩过 `mask` / `filter` 掉出合成层的坑，所以一律不用（条带因此走 `clip-path` 的锯齿多边形，而不是遮罩）。
