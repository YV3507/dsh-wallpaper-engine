...
// 后端对象 (SceneRenderer / 浏览器端 player) 都可驱动。
// 注: 曾计划接入的 Dawn/WebGPU 后端已废弃（原生崩溃 + 依赖更重，见
// docs/RENDERER-FEASIBILITY.md 的决策更新），其预计算缓存钩子一并移除。
 ...
 * @param {number} t       场景时间
 *
 * 剖析包装 (DSH_WE_PROFILE=1): 本函数有 GPU 链式等多条提前返回路径,
 * 故拆出实现体统一计时 (效果链总计) + 逐效果名计时, 供"砍效果"决策取数。
 */
export function runEffectPasses(backend, img, o, t) {
  if (!profileEnabled) return runEffectPassesImpl(backend, img, o, t);
  const __t = performance.now();
  try {
    return runEffectPassesImpl(backend, img, o, t);
  } finally {
    profAdd('效果链:总计', performance.now() - __t);
  }
}

function runEffectPassesImpl(backend, img, o, t) {
  let out = img;
  ...
  for (const ef of o.effects || []) {
    ...
    try {
      ...
      // 必须用 finally: 上方 GPU 命中路径是 `continue`, 行尾语句会被跳过。
      // 关闭剖析时不求值参数 (避免热路径白做字符串拼接)
