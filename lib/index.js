 ...
 */
function buildSeedScript(entryAbs) {
  try {
    const pj = JSON.parse(readFileSync(join(dirname(entryAbs), 'project.json'), 'utf8'));
    const props = pj && pj.general && pj.general.properties;
    if (!props || typeof props !== 'object') return '';
    const wire = {};
    for (const [k, v] of Object.entries(props)) {
      if (v && typeof v === 'object' && 'value' in v) wire[k] = { value: v.value };
    }
...
export function apply(ctx) {
  ...
  // (sf34c_) 的产物仍要被清扫, 否则会漏在磁盘上永不淘汰。
  pruneCacheDirBySize(transcodeCacheDir(), TRANSCODE_CACHE_MAX_BYTES, /^tc_.*\.mp4$/);
  pruneCacheDirBySize(frameCacheDir(), FRAME_CACHE_MAX_BYTES, SCENE_FRAME_CACHE_NAME_RE);
   ...
   * origin's authority.
   */
  function webFieldsFor(w, entryOk) {
    const isWeb = Boolean(w.type === 'web' && entryOk && w.fileAbs);
    return {
      webLive: isWeb,
      webLiveSrc: isWeb
        ? `${BASE}/scene-files/${tokenFor(w.fileAbs)}/${basename(w.fileAbs)}`
        : null,
    };
  }
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    // 档 3/4 不是渲染产物, 不参与渲染缓存: 3 = 工坊项目同目录的作者预览图;
    // 4 = 用户在设置里导入的自定义画面 (overrides/<id>.*)。直接给出文件路径。
    if (variant === 3) {
      const dirAbs = dirname(abs);
    ...
    // 画面来源模式: render(完整渲染, 默认) | maintexture(主纹理近似)。
    // ⚠️ **键格式兼容性 (重要)**: 默认模式 (render) 必须**沿用旧键格式**
    // `sf<ver>_<gpu>_<b64>_<mtime>`, 只在**非默认**模式下追加标志位 (→ `..._<gpu>m_...`)。
    // 理由: 给默认模式也加一位会让升级后用户**已缓存的全部帧失效** —— 那正是用户
    // 最初回退 0.7.3 的头号原因 ("更新后原本的缓存全部失效需要逐个点击重新加载")。
    // 这样既保住了 render 模式的既有缓存, 两模式之间也仍然互不命中。
    let srcSuffix = '';
    try { srcSuffix = readSettings().sceneFrameSource === 'maintexture' ? 'm' : ''; } catch { /* 默认 render */ }
    // 画面刷新档位后缀: **仅非 0 档追加** —— 0 档必须沿用旧键 (既有缓存零作废, 见上),
    // 各档之间也互不命中。
    const vSuffix = variant ? '_v' + variant : '';
    const key = PIPELINE_VERSION + '_' + gpuFlag + srcSuffix + '_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime) + vSuffix;
    ...
    const fbPngPath = join(dir, key + '.fb.png');
    const fbJpgPath = join(dir, key + '.fb.jpg');
  ...
  prewarmQueue = createPrewarmQueue({
    ensure: (abs, o) => ensureSceneFrame(abs, o),
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      const method = (req.method || 'GET').toUpperCase();
      if (method !== 'GET' && method !== 'HEAD') { res.statusCode = 405; res.end('method not allowed'); return; }
      // 与 /scene-live 同理：实时渲染的资源拉取算用户活动 ⇒ 后台预热推迟到动画起来之后。
      if (prewarmQueue) prewarmQueue.noteActivity();
      let rest = '';
      try {
      ...
      if (method === 'GET' && /\.html?$/i.test(target)) {
        ...
        if (shim && html.indexOf('data-we-shim') === -1) {
          ...
          const seed = buildSeedScript(target);
  ...
  // 3c-4. Renderer diagnostics sink. WebWallGL's reportDiag() fires a pixel
