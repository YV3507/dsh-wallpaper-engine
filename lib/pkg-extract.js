...
function extractSceneMainImageVia(access, label, sceneFile) {
  ...
  // scene-manifest.js 的同名函数早就有这条拒绝 (`3D scene cannot be extracted as 2D
  // frame`), 生产用的这一份漏了 ⇒ 完整渲染一旦失败, 回退链就给出这种无意义产物;
  // 拒绝之后回退链继续走到官方 preview (作者的真实渲染图), 至少是可信画面。
...
function extractSceneMainImage(pkgData, sceneFile) {
  const access = pkgSceneAccess(pkgData);
  const frame = capFallbackFrame(extractSceneMainImageVia(access, 'pkg', sceneFile), FALLBACK_MAX_WIDTH);
 ...
 */
function extractSceneMainImageFromDir(dir, sceneFile) {
  const access = dirSceneAccess(dir);
  const frame = capFallbackFrame(extractSceneMainImageVia(access, 'scene', sceneFile), FALLBACK_MAX_WIDTH);
