...
// Vendored jpeg-js (relative import) - see lib/pkg-extract.js for why the bare
// specifier cannot be used under DSH 2.0.14+ link: installs.
...
/**
 * 提取期取消探针 (Task 1): 本文件的解码器是主线程同步实现 (无 worker 可 kill),
 * AbortSignal 无法抢占正在执行的调用 —— 只能在每个昂贵步骤之间的"自然让出点"
 * 检查。措辞与 scene-frame 的 worker 路径一致 ('cancelled')。
 */
 ...
 * opts.signal (可选): 中止信号; 不传时签名与行为与旧版完全一致。
 */
function extractSceneVideoVia(access, signal) {
    const candidates = [];
    const texPaths = access.listTexPaths();
    // 自然让出点: 目录树遍历 / 整包索引解析之后 (listTexPaths 自身可能很慢)
    throwIfAborted(signal);
    for (const path of texPaths) {
        // 自然让出点: 每次读盘 + LZ4 解包一个 .tex 之前 (同步解码不可抢占)
        throwIfAborted(signal);
        const file = access.readFile(path);
        if (!file)
            continue;
        const raw = file.bytes;
        // 自然让出点: 读盘/解包之后, 进入该 .tex 的字节扫描与切片拷贝之前
        throwIfAborted(signal);
    ...
    // 自然让出点: 全部 .tex 遍历完成之后 (排序/返回大 buffer 之前)
    throwIfAborted(signal);
...
/** opts.signal 可选 — parsePkg (整包索引解析) 之前也要能取消。 */
export function extractSceneVideo(pkgData, opts) {
    const signal = opts && opts.signal;
    throwIfAborted(signal);
    return extractSceneVideoVia(pkgSceneAccess(pkgData), signal);
}
export function extractSceneVideoFromDir(dir, opts) {
    const signal = opts && opts.signal;
    throwIfAborted(signal);
    return extractSceneVideoVia(dirSceneAccess(dir), signal);
}
