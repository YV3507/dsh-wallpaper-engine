...
> ⚠️ **已归档（2026-09-23）**：本文描述的「beta 场景动画」路线（`betaSceneAnim` 开关 +
> 宿主 `/scene-anim`、`/scene-anim-progress` 路由 + 客户端升级队列 / 轮询 / 探针视频 +
> worker 多帧渲染与 APNG 输出）**已整体移除** —— WebWallGL 实时渲染已是场景动画的上位
> 替代（粒子 / 脚本 / 视差 / 包内音频都在渲染页内实时执行）。本文仅作为**历史决策记录**
> 保留，其中的资产清单与推荐实现路线不再对应仓库现状。
>
> ⚠️ **后续更新（2026-09）**：文中推荐的 **WebGPU/Dawn 后端路线已废弃** ——
> `lib/we-renderer/gpu-dawn/backend.js` 与 `lib/we-renderer/glsl/wgsl.js` 已删除（原生崩溃 +
> 适用面错位），决策与理由见 `docs/archive/static-frame/RENDERER-FEASIBILITY.md` §8；文中把它列为"路线 B / 未来实现"
> 的段落仅作历史记录。
>
...
- 官方数学基准：docs/archive/static-frame/TODO.md §四 + `_refs/we-shaders/`（官方 shader 源码）+ `_refs/linux-wallpaperengine/`
  （lwe 参考实现）。
...
- 验收基线：docs/archive/static-frame/TODO.md §二 验证基线（15 场景回归集）；静态帧渲染器是动画的对照基准。
