...
/**
 * 有损路线总开关 —— 判定与附属项解析。**单一事实来源**, 且导出以便护栏直接单测
 * (见 scripts/verify-prewarm.mjs 的真值表)。
 *
 * 语义 (用户确认):
 *   · 总开关默认关; 关闭时附属项**一律失效** (不是"保留各自的值")。
 *   · 打开时附属项**默认开** (主纹理近似); 用户显式改回 render 时尊重其选择。
 *   · 迁移: 旧配置只有 sceneFrameSource='maintexture' (没有总开关字段) 时自动打开
 *     总开关 —— 否则那些用户会被**静默**切回完整渲染 (画面变好但明显变慢)。
 *     只在字段**缺失**时迁移: 用户显式关掉总开关后不得被重新打开。
 *   · GPU 渲染加速 (sceneGpuAccel) **不在其内**: 它是独立开关, 并入会让默认配置
 *     丢掉 ~12% 性能 (实测效果链 GPU 快 1.36×)。
 */
export function lossyRouteOn(o) {
  return o.sceneLossyRoute === true
    || (o.sceneLossyRoute === undefined && o.sceneFrameSource === 'maintexture');
}

/** 附属项: 画面来源模式 render(完整渲染) | maintexture(主纹理近似)。 */
export function effectiveFrameSource(o) {
  if (!lossyRouteOn(o)) return 'render'; // 总开关关闭 → 附属项强制失效
  return o.sceneFrameSource === 'render' ? 'render' : 'maintexture';
}
...
export function sanitizeSettings(raw) {
  ...
  return {
    ...
    // 有损路线总开关 (判定/迁移语义见 lossyRouteOn 的注释)
    sceneLossyRoute: lossyRouteOn(o),
    // 画面来源模式 = 有损路线的**附属项** (解析见 effectiveFrameSource)。
    // 近似模式**不含角色骨骼合成/粒子/效果**, 且遇到含 puppet 的场景 (图集会被整张
    // 贴出) 或提取失败时自动回退真实渲染 —— 它是观感取舍, 不是"等价但更快"。
    sceneFrameSource: effectiveFrameSource(o),

    // 场景实时渲染 (WebWallGL live WebGL): 默认开启, 失败由客户端自动降级。
    sceneLive: o.sceneLive !== false,
    // 静态帧链总开关（父级是 sceneLive）：默认开。关闭时**不做静态帧渲染**，
    // 排队预热也随之停摆（它只产渲染产物）。
...
function sceneFrameCachePaths(abs, mtime, variant) {
  ...
  let srcSuffix = '';
  try { srcSuffix = readSettings().sceneFrameSource === 'maintexture' ? 'm' : ''; } catch { /* 默认 render */ }
  // 显式 static 档（8）与 auto（0）是**同一个产物**（渲染帧）：必须共用同一缓存槽，
  // 否则同一张帧被渲染两次、且 GPU 抓帧回填只对其中一个生效（见 §8 的 mp4/static 断言）。
  const key = PIPELINE_VERSION + '_' + gpuFlag + srcSuffix + '_' + Buffer.from(abs, 'utf8').toString('base64url')
...
function gpuFrameFileFor(abs, variant, slot) {
  ...
  // RENDER-FALLBACK-MODES.md §8）；3=预览图作为来源已删除，但历史缓存键可能仍在，
  // 一律返回 null 不再服务。上游原版是「档 0–3 通吃」，此处按新值域收紧。
  if (variant === 1 || variant === 2 || variant === 3) return null;
...
export function apply(ctx) {
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    const { key, srcSuffix, dir, pngPath, jpgPath, fbPngPath, fbJpgPath, gpuPath } = sceneFrameCachePaths(abs, mtime, variant);
    ...
    // 档 3 = 工坊项目同目录的作者预览图（jpg/png/gif）。**必须放在抓帧检查之后**：
    // 抓帧是真实渲染帧，档 0–3 全局优先（上游 review 的语义，见 verify-scene 的
    // 『GET ?v=3 also serves the GPU frame』）；档 4 自定义画面在上面已提前返回。
    if (variant === 3) {
      const dirAbs = dirname(abs);
      for (const fn of ['preview.jpg', 'preview.png', 'preview.gif']) {
        const p = join(dirAbs, fn);
        if (existsSync(p)) return { fileAbs: p, servedFrom: 'variant-preview' };
      }
      const err = new Error('no-preview');
      err.renderFailed = true;
      throw err;
    }
    ...
    if (!entry) {
      ...
      const pending = (async () => {
        ...
        // ── 主纹理模式: 先试"主纹理近似", 不可用则**回退真实渲染** (绝不返回空) ──
        // 不可用的判据: 无产物 (质量门拒收 / 无候选) 或 **hasPuppet**。
        ...
        if (srcSuffix === 'm') {
          try {
            mainTexFrame = abs.toLowerCase().endsWith('.json')
            ...
            mainTexFrame = null;
            gpuDiag('[framesource] 主纹理提取失败 → 回退真实渲染:',
              abs.split(/[\\/]/).slice(-2).join('/'), String(e && e.message ? e.message : e));
          }
          if (isMainTextureUsable(mainTexFrame)) {
            // 一等公民产物: 写正式缓存路径 (与"回退兜底帧" .fb. 区分开)
            const target = mainTexFrame.mime === 'image/jpeg' ? jpgPath : pngPath;
            await atomicWriteFileP(target, mainTexFrame.bytes);
            gpuDiag('[framesource] 主纹理模式命中:',
              abs.split(/[\\/]/).slice(-2).join('/'), mainTexFrame.width + 'x' + mainTexFrame.height);
            return target;
          }
          gpuDiag('[framesource] 主纹理不适用 → 回退真实渲染:',
            abs.split(/[\\/]/).slice(-2).join('/'),
            mainTexFrame ? 'hasPuppet (图集会被整张贴出)' : '无产物 (质量门拒收/无候选)');
          // 落到下面的真实渲染 (mainTexFrame 保留给 .fb. 兜底复用, 避免重复提取)
        }
  ...
  function prewarmWanted() {
    try {
      ...
      return st.scenePrewarm === true && st.sceneFrameRender !== false;
    } catch { return false; }
  }
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      //   1 = 主纹理单张（跳过合成）· 2 = 作者原画优先
      //   3 = 作者预览图（客户端内部用：静态帧渲染关闭时改显它）
      //   4 = 自定义画面（用户截屏导入）
      //   5 = 已退役（静态帧链总开关 → 设置项 sceneFrameRender）
      //   6 = 已退役（「合成」= 自动链自己的提取步，摆成手动档等于重试刚失败的那一步）
      // 5/6 旧客户端若仍传，一律按 0（自动）处理，避免留下语义已废的 _vN 缓存键。
