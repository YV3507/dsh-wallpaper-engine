...
await runScenario('A. live 首帧超时回退：探针不得留在领养槽位里', {
  ...
  check('提交后层是 live iframe + poster（buildMedia 选了 live 分支，不消费槽位）',
    !!layer && !!layer.querySelector('iframe.we-live-iframe') && !!layer.querySelector('img'),
...
await runScenario('D. web 壁纸 live 失败后必须重建为旧链', {
  ...
  t.setStats('tok-w1', { fps: 0, running: true }); // 运行中但永不出帧
  t.clock.offset = 16000;                          // 跨过运行期首帧看护的 15s
  if (tick) tick.fn();                             // → liveFail('timeout')
