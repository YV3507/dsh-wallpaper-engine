...
export function apply(ctx) {
  ...
  pruneCacheDirBySize(frameCacheDir(), FRAME_CACHE_MAX_BYTES, /^(sf34c_|san_|sv1_).*\.(png|jpg|jpeg|mp4|webm|apng)$/);
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        let gpuFlag = '0';
        try { gpuFlag = readSettings().sceneGpuAccel === true ? '1' : '0'; } catch { /* 默认 CPU */ }
        const key = 'sf34c_' + gpuFlag + '_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
        ...
        let servePath = existsSync(pngPath) ? pngPath
          : existsSync(jpgPath) ? jpgPath
            : existsSync(fbPngPath) ? fbPngPath
              : existsSync(fbJpgPath) ? fbJpgPath : null;
        if (!servePath) {
