...
export function apply(ctx) {
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    if (!entry) {
      ...
      const pending = (async () => {
        ...
        // 档 1/2 (上游 #91 的「壁纸画面刷新」备选生成逻辑): 直接用对应提取逻辑产出,
        // **不经渲染 worker** —— 它们本就是比完整渲染更轻的两种生成方式
        // (1 = 主纹理单张, 2 = 作者原画优先), 走渲染再丢弃没有意义。
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 壁纸画面刷新档位（?v=1..4，上游 #91 引入）：各档 = 不同生成逻辑，必须进
      // 缓存键；档 0/无参数沿用原键（既有帧零作废，见 ensureSceneFrame 的键兼容注释）。
      // 3 = 作者工坊预览图；4 = 自定义画面（用户导入的截屏）。
