  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        let gpuFlag = '0';
        try { gpuFlag = readSettings().sceneGpuAccel === true ? '1' : '0'; } catch { /* 默认 CPU */ }
        const key = PIPELINE_VERSION + '_' + gpuFlag + '_' + Buffer.from(abs, 'utf8').toString('base64url') + '_' + Math.round(mtime);
        ...
        let servePath = existsSync(pngPath) ? pngPath
          : existsSync(jpgPath) ? jpgPath
            : existsSync(fbPngPath) && !fallbackStale(fbPngPath) ? fbPngPath
              : existsSync(fbJpgPath) && !fallbackStale(fbJpgPath) ? fbJpgPath : null;
        if (!servePath) {
