...
// skin-center we-* modules). A separate module so the existing static-frame
// path (pkg-extract.js) is never disturbed.
...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    // 刷新/重启后记忆丢失（client 侧 sanitize 有、host 侧漏加）。
  ...
  return new Promise((resolve, reject) => {
    ...
    const runNext = () => {
      ...
      try {
        ...
        // 上不存在 → spawn ENOENT → ffmpeg 必然启动失败（cwd 必须是真实存在的目录）。
...
async function runFfmpegTranscode(abs, out, fps, signal) {
  ...
  const ENC_TUNE = {
    ...
    // 软件兜底：capped CRF（不再叠 -b:v，避免与 crf 语义打架）。一次性缓存，慢一点可接受。
  ...
  // 兜底这一档是 issue 的直接修复：没有 NVENC 的 ffmpeg 也能出片（否则功能全关）。
 ...
 *   ② **渲染逻辑一改，版本前缀必须 bump**（见下方 `SCENE_FRAME_KEY_VERSION`）——
...
export function apply(ctx) {
   ...
   * WebWallGL live-render capability, the
  ...
  // 顺延到 2 分钟：启动后立刻重提取全库静态帧会与首屏/壁纸加载抢 CPU（用户实测反馈
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      const fail = (code, payload) => {
        ...
        // 丢掉（评审 33MB 超限时客户端只看到 ECONNRESET 而非 413）。修法是
        // **lingering close** —— 先排空剩余请求体（不缓冲），**等请求体读干**再断开。
        // ⚠️ 断开时机不能挂在 res.finish 上：带着**未读的入站数据**关闭套接字会触发
        //    RST，Windows 上客户端会把已经收到的 413 一起丢掉（本机对照实测：挂
        //    res.finish ⇒ 5/5 变 status=0；等 req 'end'/'close' ⇒ 5/5 拿到 413）。
        //    排空最多等 LINGER_MAX_MS，避免「发满上限后停发」的连接被长期占住；
        //    60s 的 body idle 超时仍是最终兜底（它无条件 destroy，不经过这里）。
        const LINGER_MAX_MS = 5000;
        let lingerTimer = null;
        const drop = () => {
          if (lingerTimer) { try { clearTimeout(lingerTimer); } catch { /* ignore */ } lingerTimer = null; }
          try { req.destroy(); } catch { /* ignore */ }
        };
        if (req.readableEnded) { drop(); return; }
        if (typeof req.once === 'function') {
          req.once('end', drop);
          req.once('close', drop);
          lingerTimer = setTimeout(drop, LINGER_MAX_MS);
          if (typeof lingerTimer.unref === 'function') lingerTimer.unref();
        } else { drop(); return; }
