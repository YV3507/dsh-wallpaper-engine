window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = {
		  ...
		  // 自定义画面则用它）。此时「壁纸画面刷新」与「静态帧兜底」整组都失去意义，界面一并隐藏。
		      ...
		      React.createElement("div", { className: "we-picker__section" },
		        ...
		        (sel.type === "scene" || sel.type === "web") && !liveRenderEnabled(sel)
		          && switchRow("静态帧渲染", sel.sceneFrameRender !== false, (e) => {
		            ...
		            tooltip: "用本插件的静态帧链路渲染场景：自研渲染器输出完整场景帧（比实时渲染慢，但走的是我们自己的引擎）。关闭后场景不再渲染 —— 有内嵌 MP4 就播内嵌视频，否则显示作者预览图（导入过自定义画面则用它）。下面的「静态帧兜底」调优项只在本开关打开时出现",
		          }),
		        // ── 静态帧兜底（回退）───────────────────────────────────────────────
		        // ── 静态帧兜底（回退）───────────────────────────────────────────────
