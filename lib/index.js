...
import { createMediaBridge } from './media-bridge.js';
...
function sanitizeSettings(raw) {
  ...
  return {
    ...
    // 媒体集成（Now Playing：歌名/歌手/封面 → 壁纸的 wallpaperMediaIntegration）
 ...
 */
function liveFrameDir() { return ensureDirOnce(join(homedir(), '.dsh-wallpaper-engine', 'live-frames')); }
...
/** 给路由挂一个「非 2xx 落盘」的响应观察器（finish 时读最终状态码）。 */
function traceErrors(res, route) {
  try {
    res.on('finish', () => {
      const code = res.statusCode || 0;
      if (code >= 400) {
        appendDiagLine('http', { route, status: code, referer: String(res.req && res.req.headers ? res.req.headers.referer || '' : '').slice(0, 120), dest: String(res.req && res.req.headers ? res.req.headers['sec-fetch-dest'] || '' : '') });
      }
    });
  } catch { /* ignore */ }
}
...
export function apply(ctx) {
  ...
  // 3c-5. 媒体桥（Now Playing + 系统音频频谱）。懒启动：第一次被问到数据才启动
  //       —— macOS 首次采集会弹「音频录制」授权、并需编译 Swift 工具（约十几秒），
  //       没人用就不该弹、不该花；audioSource=off 且媒体集成关闭时完全不启动。
  const mediaBridge = createMediaBridge({
    dataDir: dirname(configPath()),
    log: (m) => console.log('[wallpaper-engine][media] ' + m),
  });
  disposers.push(() => mediaBridge.stop());
  const ensureMedia = () => {
    try {
      const cfg = readConfig();
      const st = cfg && cfg.settings ? cfg.settings : {};
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Cache-Control', 'no-store');
      res.end(JSON.stringify({ ok: true, ...mediaBridge.status() }));
    },
  }));
  disposers.push(webServer.register({
    kind: 'exact',
    ...
    handler: (req, res) => {
      ...
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Cache-Control', 'no-store');
      res.end(JSON.stringify({ ok: true, bands: Array.from(mediaBridge.spectrum()) }));
    },
  }));
  disposers.push(webServer.register({
    kind: 'exact',
    ...
    handler: (req, res) => {
      ...
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Cache-Control', 'no-store');
      res.end(JSON.stringify({ ok: true, media: mediaBridge.nowPlaying(), hasArtwork: Boolean(mediaBridge.artworkFile()) }));
    },
  }));
  disposers.push(webServer.register({
    kind: 'exact',
    ...
    handler: (req, res) => {
      const f = mediaBridge.artworkFile();
  ...
  // 3c-6. Renderer diagnostics sink. WebWallGL's reportDiag() fires a pixel
