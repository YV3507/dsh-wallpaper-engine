...
// 修复 (LOW): 原先无上限 — GLSL 兜底/第三方 shader 组合多时 program 常驻显存
let _programCache = new Map();
const PROGRAM_CACHE_MAX = 48;
let _gl = null;
let _quad = null;
...
// 修复: 条目数之外再加字节上限 — 24 张 4K RGBA 纹理 ≈ 800MB 显存常驻
const TEX_CACHE_MAX_BYTES = 256 * 1024 * 1024;
let _texCache = new Map(); // texData → { tex, lastUse }
let _texCacheBytes = 0; // _texCache 中纹理字节合计 (淘汰时同步扣减)
// sf42 显存泄漏修复: 纹理淘汰/驱逐走"按尺寸复用池"而非 deleteTexture —
// ANGLE 的 deleteTexture 不立即归还显存 (sf41e 对 FBO 已证实), 多帧动画每帧
// 新 canvas → 缓存 miss → createTexture → 驱逐 delete → 驱动侧堆积数百个
// 4K 死纹理 (实测 240 帧显存逐步上升)。改为: 驱逐的纹理按 "w x h" 进复用池,
// 新上传优先取同尺寸旧纹理 re-upload (texImage2D 覆盖数据), 零新建。
let _texFreePool = new Map(); // "w x h" → [tex, ...]
const TEX_FREE_MAX = 6; // 每尺寸最多留 6 个 (动画帧尺寸稳定, 2-3 即够)
// 修复: 复用池尺寸 key 数量同样无上限 → 加全局上限 (超出真删)
const TEX_FREE_TOTAL_MAX = 24;
let _texFreeCount = 0;
function texFreeGet(gl, width, height) {
  const key = width + ' x ' + height;
  const pool = _texFreePool.get(key);
  if (pool && pool.length) { _texFreeCount--; return pool.pop(); }
  return null;
}
function texFreePut(gl, tex, width, height) {
  const key = width + ' x ' + height;
  let pool = _texFreePool.get(key);
  if (!pool) { pool = []; _texFreePool.set(key, pool); }
  if (pool.length >= TEX_FREE_MAX || _texFreeCount >= TEX_FREE_TOTAL_MAX) {
    try { gl.deleteTexture(tex); } catch { /* ignore */ } // 超上限才真删
  } else {
    pool.push(tex);
    _texFreeCount++;
  }
...
function texCacheSet(gl, texData, tex, w, h) {
  if (_texCache.size >= TEX_CACHE_MAX) evictOldest(gl);
  const ew = w || (texData && texData.width) || 0, eh = h || (texData && texData.height) || 0;
  // 修复: 增加字节上限 (原先只按条目数封顶, 24×4K ≈ 800MB) — 超限继续淘汰最旧
  while (_texCacheBytes + ew * eh * 4 > TEX_CACHE_MAX_BYTES && _texCache.size > 0) evictOldest(gl);
  _texCache.set(texData, { tex, lastUse: _texUseSeq++, w: ew, h: eh });
  _texCacheBytes += ew * eh * 4;
  return tex;
}
function evictOldest(gl) {
  ...
  if (oldestKey != null) {
    ...
    // 复用池化 (不 deleteTexture — ANGLE 延迟归还显存, 多帧动画驱动侧堆积)
    texFreePut(gl, e.tex, e.w || 0, e.h || 0);
    _texCache.delete(oldestKey);
    _texCacheBytes -= (e.w || 0) * (e.h || 0) * 4;
    if (_texCacheBytes < 0) _texCacheBytes = 0;
  }
}
let _texUseSeq = 0;
// sf42 诊断: 累计新建纹理计数 (复用池命中不计数) — 验证显存不再螺旋
let _texCreatedTotal = 0;
function resetTexCreated() { _texCreatedTotal = 0; }
...
// 修复: 尺寸 key 数量原先无上限 (每 key 8 个 → 多种分辨率/multipass 下 FBO 无界累积)
const FBO_POOL_TOTAL_MAX = 24;
let _fboPoolTotal = 0;
...
function releaseFbo(gl, fbo) {
  if (!fbo || fbo.released) return; // 幂等: 防同一 FBO 被重复放回池 (会双份借出)
  fbo.released = true;
  // 修复: 全局上限 — 超出先淘汰最旧尺寸 key 的池内条目 (Map 插入序 = key 出现序)
  while (_fboPoolTotal >= FBO_POOL_TOTAL_MAX && _fboPool.size) {
    const oldestKey = _fboPool.keys().next().value;
    const victimPool = _fboPool.get(oldestKey);
    const victim = victimPool.pop();
    _fboPoolTotal--;
    if (!victimPool.length) _fboPool.delete(oldestKey);
    try { gl.deleteTexture(victim.tex); } catch { /* ignore */ }
    try { gl.deleteFramebuffer(victim.fbo); } catch { /* ignore */ }
  }
  const pool = _fboPool.get(fbo.key);
  if (!pool) { _fboPool.set(fbo.key, [fbo]); _fboPoolTotal++; }
  else if (pool.length < FBO_POOL_MAX) { pool.push(fbo); _fboPoolTotal++; }
  else {
...
// 上传纹理数据到 GPU (sf42 复用池优先): 取/建纹理 + texImage2D + 参数。
// 返回 { tex, unit } — unit 由调用方指定 (调用前切 activeTexture)。
function uploadTextureData(gl, texData, unit) {
  ...
  // 缓存 miss: 优先从复用池取同尺寸纹理 (re-upload, 不新建 → 防驱动侧堆积)
  t = texFreeGet(gl, texData.width, texData.height);
  if (!t) { t = gl.createTexture(); _texCreatedTotal++; }
  gl.activeTexture(gl.TEXTURE0 + unit);
  gl.bindTexture(gl.TEXTURE_2D, t);
  ...
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
  texCacheSet(gl, texData, t, texData.width, texData.height);
  return t;
}
...
export function buildProgram(gl, fragPre, vertPre) {
  ...
  // 修复 (LOW): program 缓存封顶, 超出按插入序淘汰最旧 (并释放 GL program)
  if (_programCache.size >= PROGRAM_CACHE_MAX) {
    const oldest = _programCache.keys().next().value;
    if (oldest !== undefined && oldest !== key) {
      try { gl.deleteProgram(_programCache.get(oldest)); } catch { /* ignore */ }
      _programCache.delete(oldest);
    }
  }
...
export function runEffectOnGL({ fragPre, vertPre = null, u = {}, width, height }) {
  ...
  for (const [name, texData] of texEntries) {
    ...
    if (unit === undefined) {
      unit = nextUnit++;
      uploadTextureData(gl, texData, unit);
  ...
  // 修复: FBO/colorTex 原先只在成功路径 releaseFbo — bindAttributes/drawArrays/
  // readPixels 抛错时池化 FBO 与其颜色纹理永久丢失 (effects.js 会吞掉异常回退 CPU)
  try {
  gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
  ...
  return { width, height, rgba: out };
    ...
    // 修复: 抛错路径同样归还 (releaseFbo 幂等, 正常路径与上面等价)
    releaseFbo(gl, fboObj);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  }
}
...
export function runEffectChainOnGL(effects, width, height) {
  ...
  const fboA = mkFbo();
  let fboB = null;
  // 修复: 链中任一步抛错 (bindAttributes/drawArrays/readPixels) 也必须归还两块
  // FBO — 旧实现只在成功路径 release, 每次失败泄漏一组 FBO+颜色纹理
  try {
  fboB = mkFbo();
  ...
  return { width, height, rgba: out };
    ...
    // 清理: FBO/colorTex 归还复用池 (防显存螺旋) — 抛错路径同样归还
    releaseFbo(gl, fboA);
    if (fboB) releaseFbo(gl, fboB);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  }
}
...
export function disposeGPU() {
  try {
    if (_gl) {
      ...
      for (const [, pool] of _texFreePool) {
        for (const tex of pool) { try { _gl.deleteTexture(tex); } catch { /* ignore */ } }
      }
  ...
  _fboPoolTotal = 0;
  _whiteTex = null;
  // 修复: 显式销毁 headless-gl 上下文 (GC 回收不及时, 上游要求手动销毁) —
  // 原先 disposeGPU 只把 _gl 置 null, 原生上下文与其显存一直存活到进程退出
...
// 诊断 (sf42): GPU 对象存量统计 — 定位显存泄漏 (live texture/FBO/program
// 随帧增长 = 泄漏; 稳定 = 池化生效)。
export function gpuObjectStats() {
  let texLive = 0, texBytes = 0;
  for (const [, e] of _texCache) {
    texLive++;
    if (e.w && e.h) texBytes += e.w * e.h * 4;
  }
  let freeTex = 0, freeBytes = 0;
  for (const [key, pool] of _texFreePool) {
    freeTex += pool.length;
    const m = /^(\d+) x (\d+)$/.exec(key);
    if (m) freeBytes += Number(m[1]) * Number(m[2]) * 4 * pool.length;
  }
  let fboLive = 0, fboBytes = 0;
  for (const [key, pool] of _fboPool) {
    fboLive += pool.length;
    const m = /^(\d+) x (\d+)$/.exec(key);
    if (m) fboBytes += Number(m[1]) * Number(m[2]) * 4 * pool.length;
  }
  return {
    texCache: _texCache.size,
    texLive, texBytesMB: +(texBytes / 1048576).toFixed(1),
    freePoolTex: freeTex, freePoolMB: +(freeBytes / 1048576).toFixed(1),
    fboPool: fboLive, fboBytesMB: +(fboBytes / 1048576).toFixed(1),
    programCache: _programCache.size,
    whiteTex: _whiteTex ? 1 : 0,
    texCreatedTotal: _texCreatedTotal,
  };
}
