...
export function apply(ctx) {
  ...
  async function ensureSceneFrame(abs, opts = {}) {
    ...
    // 档 5 已退役（原「强制完整渲染」）：静态帧链的总开关现在是「静态帧渲染」
    // （设置项 sceneFrameRender，父级是「场景实时渲染」），不在这条生成档位轴上。
    // 旧客户端若仍传 v=5，一律按 0（自动）处理，避免留下一个语义已废的 _v5 缓存键。
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      //   5 = **已退役**：静态帧链的总开关现在由「静态帧渲染」设置项（sceneFrameRender）
      //       承担，不属于生成档位轴；旧客户端若仍传 v=5 一律按 0（自动）处理，
      //       避免留下语义已废的 _v5 缓存键。
