...
// 排队预热 (目标 G1: 切换壁纸立即看到画面) — 队列语义与守卫见 lib/scene-prewarm.js
import { createPrewarmQueue } from './scene-prewarm.js';
...
// 定时器改由 apply() 启动 (旧实现模块级启动 + 模块级守卫 + 从不清理):
// 每次 HMR / 插件重载都会多出一个永久 200ms 定时器, 并钉住旧模块实例的
// Map/Set → 句柄返还给 apply, 卸载时 clearInterval。
let _evtLagStart = null;
function startEventLoopMonitor() {
  // 同一模块实例重复 apply: 先清掉上一份, 保证只有本次 apply 的定时器存活。
  if (_evtLagStart) { try { clearInterval(_evtLagStart); } catch { /* ignore */ } _evtLagStart = null; }
  let last = Date.now();
  _evtLagStart = setInterval(() => {
    ...
    if (gap > 100) {
      try {
        ...
        appendGpuDiagP(p, line);
      } catch { /* ignore */ }
    }
  }, 200);
  return _evtLagStart;
}
// gpu-diag.log 大小上限: 每次 >100ms 卡顿、每条 fork 诊断都 append 一行,
// 长期运行会无上限增长 → append 前查一次大小, 超过 5MB 就整文件截断重写
// (只留本次内容)。stat 失败 (文件还不存在) 时按 append 处理。
const GPU_DIAG_MAX_BYTES = 5 * 1024 * 1024;
async function appendGpuDiagP(p, text) {
  let tooBig = false;
  try { tooBig = (await stat(p)).size > GPU_DIAG_MAX_BYTES; } catch { /* 不存在 → append */ }
  try { await writeFileP(p, text, tooBig ? undefined : { flag: 'a' }); } catch { /* ignore */ }
}
 ...
 * 导出供护栏 (scripts/verify-web-route.mjs) 取真实官方项目做端到端取证 —— 探测
 * 顺序/注册表/WSL 路径规则只应有一处事实来源, 护栏不复制这套逻辑。无副作用
 * (全是只读探测), 未安装 WE 时返回 null。
 */
export async function locateWallpaperEngineP() {
  const candidates = [];
  const libraries = [];
...
// 导出供护栏断言真值表 (scripts/verify-scene.mjs Level C): 纯函数, 无副作用。
// 与 lossyRouteOn/effectiveFrameSource 同一惯例 —— 判定逻辑只有一处事实来源。
export function inferType(file) {
  if (/\.(mp4|webm|mkv|avi|mov)$/i.test(file)) return 'video';
  if (/\.(html?|js)$/i.test(file)) return 'web';
  // 应用型壁纸: WE 的 application 项目入口是 .exe (官方 defaultprojects/sheep
  // 的 project.json **没有 type 字段**却只有 sheep.exe)。旧实现把 .exe 落到
  // scene ⇒ "无损渲染"对一个不可渲染的 Unity 程序发出渲染请求, 且主纹理路径
  // 把 2MB 的 exe 当 PKG 解析 (实测 pkg: invalid string length 9460301)。
...
/**
 * Web (HTML) 壁纸的**子资源**解析 —— 纯函数 (只读文件系统), 导出供护栏断言真值表
 * (`scripts/verify-web-route.mjs`), 与 inferType / lossyRouteOn 同一惯例。
 *
 * 为什么需要它: 一张 web 壁纸是一个**多文件 HTML 应用** (官方 corsair_collection /
 * corsair_o_tron 都是 Angular / bundler 产物), 入口 index.html 里全是相对路径
 * (`main.<hash>.js` / `./js/main.js` / `images/…`)。iframe 的 src 决定这些相对
 * 路径的解析基准 —— 旧实现用 `${BASE}/media/<入口 token>`, 该 URL 没有尾斜杠,
 * 于是基准是 `${BASE}/media/`, 每个子资源都变成 `${BASE}/media/<文件名>`, 而
 * /media 只按「单文件 token」查表 ⇒ **全部 404** ⇒ 脚本/样式永远下不来 ⇒ 应用不
 * bootstrap, iframe 里什么都没有 (深色底透出来, 看起来就是"黑屏")。
 * 实测: corsair_collection 6/6、corsair_o_tron 4/4 相对资源不可达。
 *
 * 本函数把「token + 子路径」解析成真实的磁盘文件, 资源根 = **入口 HTML 所在目录**
 * (WE 项目里 file 就写在项目根, 故即项目目录; 打包器的引用一律向下, 因此不需要
 * 把资源根再往上提)。安全边界有两条, 缺一不可:
 *   ① 只有入口是 .html/.htm 的 token 才有资源根 —— 即 WE 的 web 项目目录。
 *      自上传只收 JPG/PNG/MP4 (UPLOAD_EXT), 永远拿不到这个能力; 场景/视频壁纸的
 *      token 也进不来。没有这条, 任意 media token 都能读到它同目录的其它文件。
 *   ② 解析结果必须仍在根内 —— 浏览器会把 `../` 归一化掉, 但脚本可以主动 fetch
 *      编码过的 `%2e%2e%2f`, decodeURIComponent 之后就变回 `../` (见负面用例)。
 *
 * @param {string|undefined} entryAbs 入口 HTML 的绝对路径 (来自 mediaMap)
 * @param {string} subpath URL 里 token 之后的相对子路径 (未含前导斜杠)
 * @returns {{ok: true, abs: string} | {ok: false, reason: 'bad-entry'|'bad-subpath'|'escape'|'not-found'|'not-a-file'}}
 */
export function resolveWebAsset(entryAbs, subpath) {
  if (typeof entryAbs !== 'string' || !entryAbs || !/\.html?$/i.test(entryAbs)) {
    return { ok: false, reason: 'bad-entry' };
  }
  if (typeof subpath !== 'string' || !subpath || subpath.includes('\0')) {
    return { ok: false, reason: 'bad-subpath' };
  }
  let root;
  let target;
  try {
    root = resolve(dirname(entryAbs));
    target = resolve(root, subpath);
  } catch {
    return { ok: false, reason: 'bad-subpath' };
  }
  // 包含性检查用 relative() (而非 startsWith(root + sep)): 它按平台语义处理大小写/
  // 盘符/UNC, 且 `C:foo` 这类盘符相对路径会被解析到别处后自然落到 '..'/绝对路径。
  const rel = relative(root, target);
  if (rel.startsWith('..') || isAbsolute(rel)) return { ok: false, reason: 'escape' };
  let st;
  try { st = statSync(target); } catch { return { ok: false, reason: 'not-found' }; }
...
function mimeFor(absPath) {
  ...
  return {
    ...
    // 多文件 HTML 壁纸 (web 类型) 的子资源类型 —— 见 /wallpaper-engine/web 路由。CSS 必须
    // 是 text/css: Chromium 会拒收 MIME 不是 text/css 的样式表 (标准模式 + 无
    // quirks 回退), 落到 application/octet-stream 的 CSS 等于没有样式。
    // 上游补齐了网页壁纸常用集合 (txt/xml)，本 fork 另加 svg/ico/bmp；css/json/wasm
    // 已在上方映射，此处不再重复声明（同一对象里的重复键是隐患）。
    svg: 'image/svg+xml', ico: 'image/x-icon', bmp: 'image/bmp',
    txt: 'text/plain', xml: 'application/xml',
    woff: 'font/woff', woff2: 'font/woff2', ttf: 'font/ttf', otf: 'font/otf',
    eot: 'application/vnd.ms-fontobject', wasm: 'application/wasm',
    txt: 'text/plain', map: 'application/json', xml: 'application/xml',
    // 包内音频（上游 #103）：场景 / 网页壁纸自带的 BGM / 音效。
 ...
 *
 * Encoding must stay BOM-free: Node's JSON.parse does NOT strip a UTF-8 BOM, so
 * a BOM'd JSON file fails to load. Real incident — PowerShell 5.1's
 * `Set-Content -Encoding UTF8` writes a BOM by default; a profile manifest
 * written that way made DSH Desktop abort with
 * `SyntaxError: Unexpected token '\uFEFF'` at finalStage: profile-composition.
 * PS 5.1 must use `[IO.File]::WriteAllText($p, $s, (New-Object Text.UTF8Encoding($false)))`
 * (PS 7+: `Set-Content -Encoding utf8NoBOM`) for any file Node will parse.
 */
...
/**
 * 按壁纸记忆的映射（出图来源档位 / 自定义画面标记）的白名单参数。
 * 键是壁纸 id：Steam 数字 id（`3669681034`）、自上传（`up-xxx`）、上传目录（`up-dir-xxx`）。
 * 条数上限防止配置被无限灌大（正常用户几十张，200 足够松）。
 * ⚠️ 这两份映射必须整体走白名单 —— 见 sanitizeSettings 里 frameVariants 处的教训注释。
 */
const WALLPAPER_ID_RE = /^[A-Za-z0-9_-]{1,64}$/;
const FRAME_VARIANT_MAX_ENTRIES = 200;
/**
 * 回退形态 id 的合法值域（docs/RENDER-FALLBACK-MODES.md §2）：
 *   0=auto(链头) · 4=custom(自定义画面) · 7=mp4(作者内嵌 MP4) · 8=static(静态帧渲染)
 *   · 1=maintex(单张大图) · 2=art(内嵌 JPEG/PNG)。
 * 3=作者预览图（作为回退来源已删除）、5/6（历史退役：5 曾把「静态帧渲染」总开关
 * 误放进档位表、6=合成）以及任何越界值，一律按 0（auto）—— 兼容优先，不写迁移。
 */
const FRAME_VARIANT_IDS = new Set([0, 1, 2, 4, 7, 8]);
function clampFrameVariant(v) {
  const n = Number.isFinite(Number(v)) ? Math.floor(Number(v)) : 0;
  return FRAME_VARIANT_IDS.has(n) ? n : 0;
}
...
// 并把探测投到后台。不同于上面两个从不淘汰的抽帧缓存, 这里给出有界 LRU。
...
// 场景 ortho 宽高比缓存 (scene-frame 渲染尺寸修正 — 视口比例 ≠ 场景
...
function readSceneJsonFast(src, sceneFile) {
  // 松散目录 (画面来源=场景项目目录): 主文件名以 project.json.file 为准,
  // scene.json 只是回退 —— 官方 defaultprojects 有 4 个用 <名字>.json。
  const names = [];
  if (sceneFile) names.push(sceneFile);
  try {
    const pj = JSON.parse(readFileSync(join(src, 'project.json'), 'utf8'));
    if (pj && typeof pj.file === 'string' && pj.file.toLowerCase().endsWith('.json')) names.push(pj.file);
  } catch { /* 无 project.json 或 src 不是目录 */ }
  names.push('scene.json');
  for (const n of names) {
    try {
      const p = join(src, n);
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const size = dv.getUint32(pos + 4, true);
        pos += 8;
        if (name === (sceneFile || 'scene.json') || (sjOffset == null && name === 'scene.json')) { sjOffset = offset; sjSize = size; }
        if (pos > buf.length) return null;
      }
...
function sceneAspect(abs) {
  ...
  try {
    ...
    const sc = readSceneJsonFast(src, abs.toLowerCase().endsWith('.json') ? basename(abs) : null);
...
async function renderSceneFrameInWorker(src, width, height, time, opts = {}) {
  ...
  return spawnRenderWorker(workerData, signal, gpuNode);
}
...
// 渲染 workerData 指定的单帧任务, resolve 最终结果。
function spawnRenderWorker(workerData, signal, gpuNode) {
  return new Promise((resolve) => {
    ...
    const finish = (v) => {
      ...
      gpuDiag('finish:', v.ok ? 'OK' : 'ERR', v.error || '', 'bytes=', v.png ? v.png.length : 0);
      resolve(v);
    };
    const onAbort = () => {
      try { if (worker) worker.terminate(); } catch { /* ignore */ }
    ...
    const onMessage = (msg) => {
      if (msg && msg.ok) {
        // sf41c: 大 PNG 不走 IPC — worker 写临时文件, 这里异步读回。
        ...
        const readFileBack = async () => {
          ...
          try {
            ...
            finish({ ok: true, png: buf, diff: msg.diff, checked: msg.checked });
          } catch (e) {
            // 读失败 (worker 崩溃 / 文件被回收) 时同样要删: 否则这个几百 MB 的
            // PNG 临时文件永久留在 tmpdir (旧实现只在成功路径 unlink)。
    ...
    // 大型场景 CPU 光栅化可能很慢, 给足超时 (单帧上限 600s)
...
// 有损路线总开关与画面来源模式已退役（docs/RENDER-FALLBACK-MODES.md §6）：两者表达的
// 就是链上的 static / maintex 两档，现由「出图来源」枚举直接选择 —— 不再有全局模式，
// 也不再需要"总开关 + 附属项"这层容器。GPU 渲染加速（sceneGpuAccel）始终是独立开关。

/**
 * 实时渲染在用 ⇒ 静态帧链没有显示用途，**且排队预热必须停摆**（导出仅供护栏单测，
 * 见 scripts/verify-prewarm.mjs 的 R11b）。
 *
 * 为什么必须停摆：预热会跑**冷渲染**（4K、数百 ms~数秒、worker + GPU）。当屏幕上
 * 正在跑实时渲染（动画）时，这份负载与它抢 CPU/GPU —— 用户实测症状是**卡顿**，
 * 以及实时渲染首帧超时被判失败（`liveFail` 写进失败记忆，此后该壁纸一直不实时渲染），
 * 图还没出来时的**黑屏**也来自同一条链（客户端垫底图已改为作者预览图，见 src/client.js
 * 的 isLive 分支）。
 *
 * 判据与客户端 `liveRenderEnabled()` 同口径（src/client.js）：
 *   · `sceneLive === false`（实时渲染总开关关掉）⇒ **不**抑制：此时静态帧才是显示形态；
 *   · 该壁纸已在 `sceneLiveFailures` 里（实时渲染判失败）⇒ **不**抑制：它正需要静态帧；
 *   · 场景判定用 `.pkg`（宿主 inventory 的 `sceneLive` 亦由 `isPkg` 决定，见本文件 2925 行附近）；
 *     网页/视频/图片壁纸不预热静态帧，谈不上抑制。
 *
 * @param {object} st  已 sanitize 的设置对象
 * @param {string|null} abs 当前选中壁纸的宿主侧绝对路径（由 absFromMediaUrl 解析；解析不到传 null）
 */
export function liveSuppressesPrewarm(st, abs) {
  try {
    if (!st || typeof st !== 'object') return false;
    if (st.sceneLive === false) return false;
    const id = typeof st.id === 'string' ? st.id : '';
    const fails = st.sceneLiveFailures;
    if (id && fails && typeof fails === 'object' && fails[id]) return false;
    return typeof abs === 'string' && /\.pkg$/i.test(abs);
  } catch { return false; }
}

/**
 * 设置白名单解析（**导出仅供护栏单测**，见 scripts/verify-settings-keys.mjs 的往返测试：
 * 客户端 serializeSelection() 发出的每个键都必须能从宿主白名单里原样出来）。
 */
export function sanitizeSettings(raw) {
  ...
  return {
    ...
    // GPU 渲染加速: **独立开关** (原先附属 beta场景动画, 而 beta 已搁置 ⇒ 永远传不出
    // gpuAccel=true, 是个空开关)。渲染器侧 gpuAccel 已是实装能力: 无 GPU/无驱动时
    // 自身熔断并回退 CPU, 因此不再需要 beta 做前置门控。
    sceneGpuAccel: o.sceneGpuAccel === true,
    // 空闲预热 (G1: 切换壁纸立即看到画面)。默认关闭 —— 它会写盘并在空闲时占用
    // 一个渲染进程, 必须由用户知情选择。范围: 'recent' (当前+MRU 前 6) | 'all'。
    // DSH_WE_NO_PREWARM=1 可在诊断时强制关闭 (见 lib/scene-prewarm.js)。
    ...
    // 静态帧链总开关（`sceneFrameRender`）已删除（docs/RENDER-FALLBACK-MODES.md §7）：
    // live 关掉 ⇒ **必定落在回退链的某一档**（默认 auto），不再存在「什么都不渲染」的态。
    // 排队预热的开关只剩 scenePrewarm（见 prewarmWanted）。
    ...
    userProps: (o.userProps && typeof o.userProps === 'object' && !Array.isArray(o.userProps))
      ...
      : {},
    // 按壁纸的实时渲染失败记忆 { [wallpaperId]: 'timeout'|'stall' }（客户端 heartbeat
    // 写入, 重开 sceneLive 开关时清空）。必须进白名单, 否则 PUT 上来即被丢弃、
    ...
    caretColor: typeof o.caretColor === 'string' && /^#[0-9a-f]{6}$/i.test(o.caretColor)
      ? o.caretColor : '',
    // 按壁纸记忆的出图来源档位 { [wallpaperId]: 0..4 }（客户端 serializeSelection 每次
    // PUT 都会带上；档位定义见 src/client.js 的 FRAME_VARIANTS）。
    // ⚠️ 与 sceneLiveFailures 完全同类的踩坑：**必须在白名单里**，否则 PUT 上来即被
    // 静默丢弃 —— 0.7.5 用户反馈的「画面档位永远存不住」就是这个漏字段造成的
    // （客户端每次都发、宿主每次都丢，config.json 里该字段根本不存在，刷新即回默认档）。
    // 校验不能省：这些值会进 scene-frame 的 ?v=（进而进缓存键），键名还会进对象属性。
    frameVariants: (() => {
      const s = (o.frameVariants && typeof o.frameVariants === 'object' && !Array.isArray(o.frameVariants))
        ? o.frameVariants : null;
      if (!s) return {};
      const out = {};
      for (const k of Object.keys(s).slice(0, FRAME_VARIANT_MAX_ENTRIES)) {
        if (!WALLPAPER_ID_RE.test(k)) continue;
        const v = Math.floor(Number(s[k]));
        // 合法值域 {0,1,2,4,7,8}（见 clampFrameVariant 的说明）；非数字 / 越界 / 退役值
        // （3=预览图已删、5/6=历史）一律**按 0（auto）落库** —— 显式写 0 而不是丢弃键：
        // 「无记录 = auto」与「记成 auto」对客户端等价，但显式值在诊断时看得见来源。
        out[k] = clampFrameVariant(v);
      }
      return out;
    })(),
    // 已导入自定义画面的壁纸标记 { [wallpaperId]: true }（同上：漏了就会"导入了但下次
    // 打开又变回未导入"）。只接受字面 true，避免任意值被当成"有自定义画面"。
    customFrames: (() => {
      const s = (o.customFrames && typeof o.customFrames === 'object' && !Array.isArray(o.customFrames))
        ? o.customFrames : null;
      if (!s) return {};
      const out = {};
      for (const k of Object.keys(s).slice(0, FRAME_VARIANT_MAX_ENTRIES)) {
        if (!WALLPAPER_ID_RE.test(k)) continue;
        if (s[k] !== true) continue;
        out[k] = true;
      }
      return out;
    })(),
...
function spawnFfmpeg(ff, args, timeoutMs, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // 的子进程 — 无人取消, 一直编码到 15 分钟超时。abort 后再挂监听不会触发,
    // 由下面的 spawn 后复查兜底。
    ...
    const runNext = () => {
      ...
      let proc = null;
      try {
        // cwd 跨平台修复 (移植自 fork 2081b4b): Windows 用 SystemRoot (console-app
        // 启动稳定性); 非 Windows 上 'C:\\' 不存在 → spawn ENOENT → ffmpeg 永远
        // 启动失败, 故回退 undefined (继承父进程 cwd)。
      ...
      // 结束整个尝试链 (不再 spawn 第二次), 避免切换壁纸后它继续编码 15 分钟。
      if (signal && signal.aborted) {
...
// 场景帧缓存键版本 (PIPELINE_VERSION): 真实渲染产物 (key.png|jpg) 与回退主纹理
// 产物 (key.fb.png|jpg) 的名字里都带它 —— **只要场景渲染管线/worker/提取语义
// 变化就必须自增**, 否则磁盘上的旧产物会继续被命中: 特别是回退主纹理, 用户升级
// 到渲染器修好的版本后仍然只能看到旧的主纹理巨图 (上游 #86)。原先这里是路由内的
// 手工字面量 'sf34c_' (历史: sf34 → sf34b → sf34c → 本常量)。
// sf36a (2026-10-03): 本轮渲染器正确性修复必须让**磁盘上的旧帧**全部失效,
// 否则用户升级后仍命中旧缓存 (暗帧 / 马赛克) 并以为没修:
//   · 正交静态 camera.eye 不再平移 2D 图层 (Eagle Flag 左侧 583px 灰带)
//   · beach 的 flowimage 变体 B (背景整幅纯黑) / eagleflag 法线贴图按 TEX1FORMAT 解码
//   · .mdl 多子网格解析 (Arsenal 只剩一把刀) / 透视场景粒子投影 (dna_fragment 粒子零产出)
//   · tint 退化判据纳入 alpha
//   · hdr=true 的 bloom 段补 linear→sRGB (razer_bedroom 曾被压暗 4.4×)
//   · 绘制采样改为"放大双线性 / 缩小盒式预滤波" (背景马赛克)
//   · 三维场景自定义着色器与 generic 光照公式 (ricepod 全黑) + 粒子稳态取帧 (shimmering 回退缩略图)
// sf42a (本轮): 静态帧取样时刻由"固定 t=2.5"改为**按内容度量选点** (worker 侧
//   两段式: 请求时刻先按生产尺寸渲染并按内容度量判健康 → 健康则原样采用; 不健康才
//   在小探针帧上量化候选时刻选点, 且必须"显著更好"才切换)。键里既不含时刻也不含
//   选点语义, 故**必须升版**: 否则 t=2.5 是暗帧/空白的场景 (arsenal 均值 3.7/255、
//   audiophile 开场平色、shimmering_particles 全清屏) 会继续命中磁盘上的旧帧。
//   代价: 全部场景帧按需重新渲染一次 (健康场景的渲染路径与旧版逐字节相同, 只是换了
//   键名; 排队预热队列负责分摊)。细则与实测见 .test-cache/fix-sampling-time.md。
// sf42b: 追加**官方 preview 画像校准** (阶段二): 当前帧已过健康门禁、但与该场景
//   preview.jpg 的 1:1 中心方裁画像仍远离 (|Δmean|>25% 或 |Δlit24|>20pp) 时, 用参考
//   画像距离做"粗扫 + 局部细扫"重选 (arsenal t=200 → t≈183: 中心方裁均值 23.9 → 45.4,
//   官方 63.3; lit24 23.7% → 65.6%, 官方 73.2%); 并加上界/自洽性防反例 —— retro 的
//   preview 逐通道超出其 bg.frag 可证上界 (schemecolor 0.72/0.64/0.42 = 184/163/107,
//   preview 蓝通道均值 141.2 > 107) ⇒ 该场景不参与校准、退回健康判据。选点结果再次
//   变化 ⇒ 再次升版。
// sf42c: **官方 preview 画像校准改为默认关闭**, 并把"候选重选"完全落在壁纸自带数据上:
//   原生行为审计 (.test-cache/native-oracle-audit.md §1.3/C7/S1) 证明 9/9 点名场景的
//   preview 都是方形缩略图 (side 207–1080), 而场景正交画布是 1.778/2.333/5.333 ⇒
//   **结构上不是帧**, 不是 WE 原生输出; Retro 的 preview 还逐通道超出其自带 bg.frag
//   导出的解析上界 (184/163/107)。用户方针是"行为对照 WE 原生渲染" ⇒ 默认只允许
//   "壁纸自带数据 (相机路径时长/粒子 starttime+lifetime) + 渲染帧自身内容度量"参与选点;
//   preview 校准保留为实验开关 DSH_WE_PREVIEW_CAL=1。同轮阶段一增加**局部细扫**
//   (围绕粗扫最优, 区间由相邻候选间距与相机路径总长给出, 评分仍只用内容度量) ⇒
//   退化场景 (如 arsenal) 不必依赖 preview 也能取到内容更优的时刻。选点结果再次变化 ⇒ 再次升版。
// sf44: 材质着色器改走数据驱动 (lib/we-renderer/glsl/material.js 解释器接手 generic 族
//   + genericimage2 图像层)。arsenal 的像素变化已实测 (覆盖 43.49%→85.36%, 均值 7.52→4.03;
//   成因与逐点取证见 .test-cache/fix-glsl-materials.md §5.1) ⇒ 缓存键再升一版。
// sf45: 补上官方 `a_TexCoordVec4.zw` (lightmap UV) 的数据源 —— parseMdlStatic 不填 uv2s,
//   解释器改从 MDL 顶点流自校验提取 (model.js:_meshUV2)。arsenal 亮度恢复
//   (t=183 5.60→15.69 / t=200 5.48→13.53 / t=2.5 4.03→10.66; 唯一色 2008→8291)
//   ⇒ 缓存键再升一版。取证: .test-cache/fix-glsl-materials.md §10。
const PIPELINE_VERSION = 'sf45';
// 回退主纹理产物的有界重试 TTL: 比这更旧的 .fb. 产物不再算缓存命中 (见 scene-frame
// 路由), 于是坏渲染器最多每 TTL 重试一次真实渲染 —— 既不是每个请求一次 (渲染是
// 分钟级的 fork), 也永远不会永久固化。每次写出新回退产物都会刷新 mtime
// (atomicWriteFileP = 先写 .tmp 再 rename), TTL 因此自动顺延, 不需要额外记账文件。
const FALLBACK_RETRY_MS = 60 * 60 * 1000;
// scene-frame 缓存文件名 (启动清扫用): 当前键前缀 **和** 历史键前缀 sf34c_ 都要认,
// 否则升版后旧键产物在磁盘上再也匹配不到 → 永不淘汰的孤儿文件。
const SCENE_FRAME_CACHE_NAME_RE = new RegExp(
  '^(' + [PIPELINE_VERSION + '_', 'sf34c_', 'san_', 'sv1_'].join('|') + ').*\\.(png|jpg|jpeg|mp4|webm|apng)$');
// 场景帧缓存路径的**唯一**构造点（渲染产物 / 回退主纹理 / GPU 抓帧共用）。
// 键 = <PIPELINE_VERSION>_<gpu 标志><来源后缀>_<base64url(abs)>_<mtime>[_vN]：
//   · 默认模式沿用旧键格式，只在非默认模式（maintexture）与档位 ≠ 0 时追加标志位
//     —— 给默认模式也加一位会让升级后用户已缓存的帧全部失效（用户最初回退
//     0.7.3 的头号原因），各档各模式之间仍然互不命中。
//   · .fb. = 回退主纹理产物（独立命名，否则分不清回退帧，且停在磁盘上的旧回退帧
//     会盖住后来成功的真实渲染帧）。
//   · _gpu.png = live 渲染页抓帧回填（文件名即标记，存在即优先上屏）。
// ⚠️ 上游原版把槽位键写成主纹理键 SCENE_FRAME_KEY_VERSION（sf34），但本 fork 的
// 静态帧是**渲染产物** ⇒ 槽位必须与渲染产物同键：否则 live 回填写在一处、GET 查
// 在另一处，抓帧永远不生效（"两处各自写字面量就会错位" 正是上游注释的警告）。
function sceneFrameCachePaths(abs, mtime, variant) {
  let gpuFlag = '0';
  try { gpuFlag = readSettings().sceneGpuAccel === true ? '1' : '0'; } catch { /* 默认 CPU */ }
  // 显式 static 档（8）与 auto（0）是**同一个产物**（渲染帧）：必须共用同一缓存槽，
  // 否则同一张帧被渲染两次、且 GPU 抓帧回填只对其中一个生效（见 §8 的 mp4/static 断言）。
  const key = PIPELINE_VERSION + '_' + gpuFlag + '_' + Buffer.from(abs, 'utf8').toString('base64url')
    + '_' + Math.round(mtime) + (variant && variant !== 8 ? '_v' + variant : '');
  ...
  return {
    ...
    fbPngPath: join(dir, key + '.fb.png'),
    fbJpgPath: join(dir, key + '.fb.jpg'),
    gpuPath: join(dir, key + '_gpu.png'),
  };
}
// scene-frame 缓存槽位解析（GET / HEAD / GPU 回填 PUT 共用）。GPU 抓帧存独立的
// <key>_gpu.png —— 文件名即标记：存在 = 该槽已被 live 渲染抓帧覆盖（拒绝重复写，
// 缓存唯一），且可直接双击打开查看。
function sceneFrameSlot(abs, variant) {
  let mtime = 0;
  try { mtime = statSync(abs).mtimeMs; } catch { /* keep 0 */ }
  return sceneFrameCachePaths(abs, mtime, variant);
}
...
function gpuFrameFileFor(abs, variant, slot) {
  // GPU 抓帧只服务**会渲染的档**：0=auto、8=static（_gpu.png 就是那一档的内容）。
  // 1=单张大图 / 2=内嵌图是**用户显式选的替代来源**，抓帧不得顶掉它们（docs/
  // RENDER-FALLBACK-MODES.md §8）；3=预览图作为来源已删除（且会被归一成 0）。
...
// transcodeCacheDir 的 tc_*.mp4 每个 80-280MB, 场景帧缓存 sf*/sf34c_*.png/jpg 与
// sv1_*.mp4 也从不淘汰 → 长期使用后无上限增长 (几十 GB)。san_*.apng|mp4 是已移除的
// beta 场景动画(betaSceneAnim/scene-anim)遗留产物, 保留在匹配式中只为把它们回收掉。
// 启动时各扫一次 (与 sweepTranscodeArtifacts 同风格: 启动路径上的同步 syscall
// 可接受), 超上限就按 mtime 从最旧开始删到上限以内。保守起见: 永不删最新的
// 一份, 永不删 5 分钟内写出的文件 (可能是在途任务的产品), 永不碰本进程的
// .tmp<pid> 输出。
const TRANSCODE_CACHE_MAX_BYTES = 4 * 1024 * 1024 * 1024;
const FRAME_CACHE_MAX_BYTES = 2 * 1024 * 1024 * 1024;
...
// 预热互斥（用户 2026-09-25 定）：上游这轮**提取式**预热与本 fork 的**渲染式**
// 预热队列不得同时进行 —— 两者都是全库级的磁盘/CPU 负载，叠加会把插件加载与切换
// 壁纸一起拖慢。上游这轮优先，我方队列在它结束前原地等待（见 prewarmQueue 的
// ensure 包装）。
let extractionPrewarmRunning = false;
/** 上游提取式预热期间让路：轮询等待，可被取消信号中断。 */
async function waitForExtractionPrewarm(signal) {
  while (extractionPrewarmRunning) {
    if (signal && signal.aborted) throw new Error('cancelled');
...
async function sceneFramePrewarm() {
  ...
  extractionPrewarmRunning = true;
  try {
    await sceneFramePrewarmBody();
  } finally {
    extractionPrewarmRunning = false;
  }
}
async function sceneFramePrewarmBody() {
...
// ── 陈条清理（docs/RENDER-FALLBACK-MODES.md §6）─────────────────────────────
// 持久化里按壁纸记的表（frameVariants / customFrames / sceneLiveFailures，以及按路径
// token 记的 userProps）会随壁纸库变动留下"陈条"：壁纸已删/已移出库，条目却永远留着。
// 定案：每次启动后**只完整跑一轮**，完全在后台（启动路径不做任何同步扫描）；每批
// **16 条**、批间**静息 30 秒**；**复用预热的空闲/活动门控** ⇒ 不与 live 渲染、预热抢
// 资源（用户一交互就继续等）；每轮写一条 gpu-diag（可解释、可诊断）。
const STALE_PRUNE_BATCH = 16;
const STALE_PRUNE_QUIET_MS = 30000;
const STALE_PRUNE_START_DELAY_MS = 90000; // 启动后 90s 再开始：避开首屏与上游提取式预热（120s 前跑完一批足够）
/**
 * 纯函数（可单测）：按"当前库的 id 集合"与"路径是否存在"收集陈条。
 * 返回 [[表名, 键], ...] —— **只报告、不改数据**（调用方分批删除并写回）。
 */
export function collectStaleEntries(settings, validIds, pathExists = existsSync) {
  const out = [];
  const valid = validIds instanceof Set ? validIds : new Set(Array.from(validIds || []).map(String));
  for (const name of ['frameVariants', 'customFrames', 'sceneLiveFailures']) {
    const m = settings && settings[name];
    if (!m || typeof m !== 'object') continue;
    for (const k of Object.keys(m)) if (!valid.has(String(k))) out.push([name, k]);
  }
  // userProps 的键是 token（base64url(绝对路径)）：**路径已不存在**即陈条（壁纸被删/移走）。
  const up = settings && settings.userProps;
  if (up && typeof up === 'object') {
    for (const k of Object.keys(up)) {
      let abs = '';
      try { abs = Buffer.from(k, 'base64url').toString('utf8'); } catch { /* 非法键：按陈条处理 */ }
      let ok = false;
      if (abs) { try { ok = typeof pathExists === 'function' ? Boolean(pathExists(abs)) : false; } catch { ok = false; } }
      if (!ok) out.push(['userProps', k]);
    }
  }
  return out;
}
/** 复用预热的空闲门控：预热在飞、或最近 STALE_PRUNE_QUIET_MS 内有过用户活动 ⇒ 让路。 */
function stalePruneIdle() {
  try {
    if (!prewarmQueue) return true;
    const st = prewarmQueue.status();
    return !st.running && (Date.now() - Number(st.lastActivityAt || 0)) >= STALE_PRUNE_QUIET_MS;
  } catch { return true; }
}
const sleepUnref = (ms) => new Promise((r) => { const t = setTimeout(r, ms); if (t && typeof t.unref === 'function') t.unref(); });
/** 启动后一轮：收集 → 分批（16/批，批间静息 30s，每批前等空闲）→ 写回。 */
async function pruneStaleWallpaperEntries() {
  const installDir = await locateWallpaperEngineP();
  const libraryDirs = await owningLibrariesP();
  const all = await enumerateWallpapersAsync(installDir, libraryDirs);
  const valid = new Set(all.map((w) => String(w.id)));
  // ⚠️ 当前库为空（没装 WE / 扫描失败）时**不清理**：否则会把整张表误删成"全陈条"。
  if (!valid.size) { gpuDiag('[prune-stale] 跳过：当前壁纸库为空（扫描失败或未安装）'); return; }
  const stale = collectStaleEntries(readSettings() || {}, valid);
  if (!stale.length) { gpuDiag('[prune-stale] 无陈条（库 ' + valid.size + ' 张）'); return; }
  let done = 0;
  let batches = 0;
  for (let i = 0; i < stale.length; i += STALE_PRUNE_BATCH) {
    // 每批之前等空闲（含第一批）：绝不与 live 渲染/预热抢资源；等待可被打断（用户活动即续等）。
    for (let waited = 0; waited < 600; waited++) {
      if (stalePruneIdle()) break;
      await sleepUnref(1000);
    }
    const cur = readSettings() || {}; // 重新读：期间用户可能改过设置（别用旧快照覆盖）
    let touched = false;
    for (const [name, k] of stale.slice(i, i + STALE_PRUNE_BATCH)) {
      const m = cur[name];
      if (m && typeof m === 'object' && Object.prototype.hasOwnProperty.call(m, k)) { delete m[k]; touched = true; done++; }
    }
    if (touched) { try { writeSettings(cur); } catch { /* 写失败不影响运行，下一轮再试 */ } }
    batches++;
    if (i + STALE_PRUNE_BATCH < stale.length) await sleepUnref(STALE_PRUNE_QUIET_MS);
  }
  gpuDiag('[prune-stale] 清理陈条 ' + done + '/' + stale.length + '（库 ' + valid.size + ' 张 · ' + batches + ' 批）');
}
...
export function apply(ctx) {
  ...
  // 事件循环延迟监控: 定时器归本次 apply 所有 — 句柄捕获在局部变量, 由下面的
  // disposer 精确清理 (见 startEventLoopMonitor 注释: 模块级启动会在每次 HMR
  // 后留下一个永不释放的 200ms 定时器)。
  const evtLagTimer = startEventLoopMonitor();
  ...
  const startupSweepTimer = setTimeout(() => {
    ...
    // 大文件缓存同样要有上限: transcode 输出 (tc_*.mp4, 每个 80-280MB) 与场景
    // 帧/视频缓存 (sf34c_*, san_*, sv1_*) 旧实现从不淘汰 → 按 mtime-LRU 收敛到
    // 上限以内 (启动一次, 详情见 pruneCacheDirBySize)。只匹配真正的媒体文件,
    // 不碰同名的 .loop / .prog / .tmp 附属文件。回退主纹理产物 sf34c_*.fb.png|jpg
    // 也属于 scene-frame 缓存条目 ('fb' 在 `.*` 覆盖范围内, 无需改模式)。
    // 模式改用 SCENE_FRAME_CACHE_NAME_RE: 键前缀升到 PIPELINE_VERSION 后, 旧前缀
    // (sf34c_) 的产物仍要被清扫, 否则会漏在磁盘上永不淘汰。定时器里跑 ⇒ 逐条
    // try/catch, 任一失败都不能把异常抛进 timer 回调（会打死宿主进程）。
    try { pruneCacheDirBySize(transcodeCacheDir(), TRANSCODE_CACHE_MAX_BYTES, /^tc_.*\.mp4$/); } catch { /* ignore */ }
    try { pruneCacheDirBySize(frameCacheDir(), FRAME_CACHE_MAX_BYTES, SCENE_FRAME_CACHE_NAME_RE); } catch { /* ignore */ }
  }, 3000);
   ...
   */
  function sceneFieldsFor(w, hasFrame, customIds, svKnown) {
    ...
    return {
      // ⚠️ 历史字段：`/scene-runtime` 的旧 WebGL 播放器入口。**当前客户端零消费**
      // （`src/client.js` 对 `sceneUrl` 无引用；类型声明的存在只为兼容旧客户端/第三方）。
      // 路由本身仍在服务（散装媒体/回退路径可能用到），所以这里继续发出；若将来确认无人
      // 使用，可连同 `lib/types/index.d.ts` 的声明一起摘掉。
      sceneUrl: isScene ? `${BASE}/scene-runtime/${tokenFor(w.fileAbs)}` : null,
      sceneLive: isPkg,
      sceneLiveSrc: isPkg ? tokenFor(w.fileAbs) : null,
      // sceneVideo 诚实化 (#92): 只有探测缓存确认该 scene 主文件真的内嵌 MP4 才给
      // URL; 未知 (undefined) → null, 由调用方投递后台探测 —— 绝不猜。旧实现用
      // isScene 冒充「有内嵌 MP4」, 客户端必然请求到 404。
      sceneVideo: isScene && svKnown === true ? `${BASE}/scene-video/${tokenFor(w.fileAbs)}` : null,
  ...
  async function buildInventory() {
    ...
    const wallpapers = await Promise.all(all.map(async (w) => {
      ...
      // 冒充, 导致客户端必然 404)。sceneUrl/frameUrl 行为不变。
      const svKey = w.type === 'scene' && hasFrame ? sceneVideoProbeKey(w.fileAbs, sceneMtime) : null;
      ...
      return {
        ...
        // 多文件 HTML 壁纸的 iframe 入口。**必须**与 media 分开: media 是单文件
        // URL (无尾斜杠 ⇒ 相对基准是 `${BASE}/media/`), 用它当 iframe src 会让入口
        // 里的每一个相对资源都 404 (实测 corsair_collection 6/6、corsair_o_tron 4/4)。
        // 本字段把入口挂在「目录型基准」下 (见 `${BASE}/web` 路由), 相对资源因此
        // 落回同一前缀。老客户端不认这个字段 → 仍用 media, 行为与修复前一致。
        webView: w.type === 'web' && hasMedia
          ? `${BASE}/web/${tokenFor(w.fileAbs)}/${basename(w.fileAbs)}`
          : null,
        ...
        // embedded MP4 (svKnown honest gate), packaged audio, 自定义画面.
        ...sceneFieldsFor(w, hasFrame, customIds, svKnown),
    ...
    const uploads = await Promise.all((await enumerateUploadsP(uploadsDir)).map(async (w) => {
      ...
      const [hasMedia, hasPreview, sceneMtime] = await Promise.all([
        ...
        w.previewAbs ? pathExistsP(w.previewAbs) : Promise.resolve(false),
        // 改用 stat 取 mtime (与 Steam 扫描同款): 既作存在性探测 (hasFrame),
        // 又充当 sceneVideo 诚实化探测缓存的键 (路径 + mtime)。
        w.type === 'scene' && w.fileAbs ? mtimeOrNullP(w.fileAbs) : Promise.resolve(null),
      ]);
      const hasFrame = sceneMtime !== null;
      // sceneVideo 诚实化 (#92): 与 Steam 扫描同一条判据 —— 只有探测缓存确认该
      // 主文件真的内嵌 MP4 才给 URL, 未知则投递后台探测。自上传/存储位置目录里
      // 的场景与工坊场景必须行为一致, 否则这里会静默退回静态帧。
      const svKey = w.type === 'scene' && hasFrame ? sceneVideoProbeKey(w.fileAbs, sceneMtime) : null;
      const svKnown = svKey === null ? undefined : sceneVideoProbeGet(svKey);
      if (svKey !== null && svKnown === undefined) scheduleSceneVideoProbe(w.fileAbs, svKey);
      const webMediaBase = w.type === 'web' && hasMedia ? await mediaOriginBase() : '';
  ...
  // §6 陈条清理：启动后**一轮**、完全后台（unref 的定时器 + 分批 16 + 批间静息 30s +
  // 复用预热的空闲门控）。挂在这里而不是启动清扫旁：`disposers` 直到上面这行才声明
  //（TDZ —— 放前面会 `Cannot access 'disposers' before initialization`，实测踩到）。
  const stalePruneTimer = setTimeout(() => {
    pruneStaleWallpaperEntries().catch(() => { /* 清理失败不影响功能 */ });
  }, STALE_PRUNE_START_DELAY_MS);
  if (typeof stalePruneTimer.unref === 'function') stalePruneTimer.unref();
  disposers.push(() => { try { clearTimeout(stalePruneTimer); } catch { /* ignore */ } });
  ...
  // 2b. Web (HTML) 壁纸的**子资源**路由 —— 没有它, web 壁纸在浏览器里永远是空白的。
  //
  // 一张 web 壁纸是多文件 HTML 应用, 入口里的资源全是相对路径。iframe 的 src 决定
  // 相对解析基准: 本路由把入口挂在 `${BASE}/web/<token>/<入口文件名>`, 基准因此是
  // `${BASE}/web/<token>/`, 于是 `main.<hash>.js` / `./js/main.js` / `images/x.svg`
  // 自然落回本前缀, 由「token + 子路径」解析到入口所在目录 (`resolveWebAsset`)。
  //
  // 为什么不是改写 HTML 注入 <base>: 不改内容就没有编码/长度/Range 的连带问题, 而且
  // 官方 corsair_collection 自带 `<base href=".">` —— 它相对本基准解析恰好指向同一
  // 目录, 不需要也不应该被替换。目录型基准同时也让 Angular 的路由基路径自洽。
  //
  // 旧行为 (回归护栏在 verify-web-route.mjs 里): iframe 指向 `${BASE}/media/<token>`
  // ⇒ 基准 `${BASE}/media/` ⇒ 子资源全部 404。
  disposers.push(webServer.register({
    kind: 'prefix',
    path: `${BASE}/web`,
    handler: (req, res) => {
      ...
      try {
        const pathname = new URL(req.url || '/', 'http://x').pathname;
        rest = pathname.startsWith(`${BASE}/web/`)
          ? decodeURIComponent(pathname.slice(`${BASE}/web/`.length))
          : '';
      } catch {
      ...
      const r = resolveWebAsset(mediaMap.get(token), subpath);
      if (!r.ok) {
        res.statusCode = r.reason === 'escape' ? 403 : 404;
        res.end(r.reason);
        return;
      }
      // no-store: token 只是 base64url(路径), **不含 mtime** —— 工坊更新后同一个
      // URL 必须拿到新字节, 否则浏览器会一直用旧的 main.<hash>.js (与 scene-frame
      // 同一条策略)。其余流式/Range/HEAD 行为复用 serveFile, 与 /media 完全一致。
      res.setHeader('Cache-Control', 'no-store');
      serveFile(r.abs, req, res, method === 'HEAD');
    },
  }));
  ...
  // ── 场景静态帧: 确保产物就绪 (路由与排队预热共用) ────────────────────────────
  // 抽取自 ${BASE}/scene-frame 路由。让**排队预热队列与按需请求走同一条代码路径**,
  // 于是"预热帧与按需帧同键同内容"是结构保证, 而不是两处各自实现后再对账 ——
  // 本项目已经出现过两处这种漂移 (pkg-extract.js 与 scene-manifest.js 的 composite
  // 各一份且已分叉, 见 docs/SCENE-FRAME-PERF.md §十一)。
  //
  // 返回 { fileAbs, servedFrom }: servedFrom ∈ 'cache' | 'render' | 'fallback' | 'render-failed'
  //
  // @param abs  场景文件绝对路径 (scene.pkg 或松散 scene.json)
  // @param opts
  //   signal        AbortSignal。**等待者计数语义与抽取前逐字一致**: 调用方 abort 时
  //                 该等待者注销; 最后一个等待者离开才取消渲染 (多请求共享一次 fork)。
  //   allowFallback 渲染失败时是否回退主纹理 (默认 true = 路由行为)。
  //                 **预热传 false**: 回退提取本身要 688ms–12.3s, 且产物是 .fb. 兜底帧,
  //                 拿它当预热结果既慢又不合语义 (也避免"interim 被当成本体"的重演)。
  //   variant       帧来源档位 (上游 #91 的「壁纸画面刷新」，本 fork 里 UI 名为「出图来源」):
  //                 0 = 默认 (自动回退链: 完整渲染 → 主纹理提取 → 作者预览图),
  //                 1 = 主纹理单张, 2 = 作者原画优先, 3 = 作者预览图, 4 = 自定义画面。
  //                 非 0 档进缓存键后缀 (见下), 各档互不覆盖; 0 档沿用旧键。
  //                 ⚠️ 只认 1..4: 历史上档 5 =「静态帧渲染」总开关 (误放进档位表)、
  //                 档 6 =「合成（分层）」, 两者均已退役, 见下面 vParsed 处的钳制。
  async function ensureSceneFrame(abs, opts = {}) {
    const signal = opts.signal || null;
    const allowFallback = opts.allowFallback !== false;
    const vRaw = Number(opts.variant || 0);
    // 档 5/6 已退役（5=「静态帧渲染」总开关、6=「合成」—— 后者正是自动链自己的
    // 提取步）。旧客户端若仍传它们，一律按 0（自动）处理，避免留下语义已废的 _v5/_v6 键。
    const vParsed = Number.isFinite(vRaw) ? Math.floor(vRaw) : 0;
    const variant = (vParsed >= 1 && vParsed <= 4) ? vParsed : 0;
    // 档 4 不是渲染产物、也不参与渲染缓存：用户在设置里导入的自定义画面
    // (overrides/<id>.*) 是**显式选择**，优先级最高（抓帧也不得顶掉它）。
    if (variant === 4) {
      const p = customFramePath(customIdFromAbs(abs));
      if (!p) {
        const err = new Error('no-custom-frame');
        err.renderFailed = true;
        throw err;
      }
      return { fileAbs: p, servedFrom: 'variant-custom' };
    }
    let mtime = 0;
    try { mtime = statSync(abs).mtimeMs; } catch { /* keep 0 */ }
    ...
    // 缓存键版本 sf34 → sf34b → sf34c: 静态帧语义共变过两次 ——
    // ① sf34b: 一次性动画改用"稳定态"时刻 (后因改到渲染器侧 staticFrame 而把时间回退);
    // ② sf34c: 时间回到 2.5s + 渲染器对 mode=single / MDL loop=single 直接取保持态。
    // 键里既不含时刻也不含语义, 不升版就会继续命中旧语义下生成的 PNG ——
    // 实测 Angel Mail 曾命中一张**全屏单色**帧 (该帧由已废弃的旧语义生成, 离线复现不出)。
    // 缓存键含 gpu 标志: GPU (WebGL) 与 CPU 输出有微小 float 精度差 —
    // sceneGpuAccel 切换时不得命中另一管线的旧帧 (sf40h)。
    // 键前缀由 PIPELINE_VERSION 常量给出 (原来写死 'sf34c_': 渲染器一变就得
    // 记得改这里的字面量, 现在只改常量)。
    // 键与各产物路径的唯一构造点（GPU 槽同键；键格式与档位兼容性见 sceneFrameCachePaths）。
    const { key, dir, pngPath, jpgPath, fbPngPath, fbJpgPath, gpuPath } = sceneFrameCachePaths(abs, mtime, variant);
    // GPU 抓帧全局优先（档 0–3 通吃；档 4 = 用户自定义画面不受影响）：live 渲染页首帧
    // 确认后回填的 <key>_gpu.png 是**真实渲染帧**，优于任何 CPU 提取档位。抓帧挂在档 0
    // 的键上，所以选过其它档位的壁纸也要用它（档 0 槽位查找见 gpuFrameFileFor）。
    // 它同时属于「缓存命中」——cachedOnly 的垫底图拿到的正是磁盘上的真帧。
    // ⚠️ 回退链改造后这里要收紧为「仅档 0（自动）」（见 docs/RENDER-FALLBACK-MODES.md）：
    // 那时 1/2 档是用户显式选定的回退级，不该被渲染帧顶掉。
    const gpuHit = gpuFrameFileFor(abs, variant, { gpuPath });
    if (gpuHit) return { fileAbs: gpuHit, servedFrom: 'gpu' };
    // 档 3（作者预览图）作为**回退来源**已删除（§5）：宿主不再服务它 —— 老客户端传来的
    // `?v=3` 由 clampFrameVariant 归一到 0（auto），因此这里也不需要分支。作者预览图仍是
    // **选择器缩略图**的资产（inventory 的 preview 字段），只是不再当回退档。
    // 回退主纹理产物独立命名 (上游 issue #86): 真实渲染 (key.png/jpg) 与
    // 回退主纹理 (key.fb.png/jpg) 分开落盘 —— 否则单看文件名分不出回退帧,
    // 且停在磁盘上的旧回退帧会掩盖后来成功的真实渲染帧。
    // (两个 fb 路径由 sceneFrameCachePaths 一并给出，见上面的解构。)
    // 读取优先级: 真实渲染产物优先 (png → jpg), 回退主纹理在后 (fb.png → fb.jpg)。
    // 回退产物有界重试 (上游 #86 第二处): 回退帧不得永久命中 —— 否则渲染器修好
    // 后用户仍会一直看到旧的主纹理巨图。超过 FALLBACK_RETRY_MS 的回退产物视为
    // miss, 于是走下面的渲染路径; 而重试一旦再次失败就会重写该文件并把 mtime
    // 刷新到现在, 所以同一壁纸最多每 TTL 重试一次。重试与普通 miss 走同一个
    // SCENE_FRAME_INFLIGHT 键, 并发请求只会共享一次渲染, 不会各起一个 fork。
    const fallbackStale = (p) => {
      try { return Date.now() - statSync(p).mtimeMs > FALLBACK_RETRY_MS; } catch { return true; }
    };
    if (existsSync(pngPath)) return { fileAbs: pngPath, servedFrom: 'cache' };
    if (existsSync(jpgPath)) return { fileAbs: jpgPath, servedFrom: 'cache' };
    // allowFallback=false (预热): 已有的有效兜底帧不算命中 —— 预热的目标是真实渲染产物
    if (allowFallback) {
      if (existsSync(fbPngPath) && !fallbackStale(fbPngPath)) return { fileAbs: fbPngPath, servedFrom: 'fallback' };
      if (existsSync(fbJpgPath) && !fallbackStale(fbJpgPath)) return { fileAbs: fbJpgPath, servedFrom: 'fallback' };
    }
    // ── 只命中、不渲染 (opts.cachedOnly) ────────────────────────────────────
    // 用途：live 的**垫底图**以及任何"有就用、没有就算了"的探测。命中判定与上面
    // 完全一致（含 `.fb.` 兜底帧的新鲜度判定），但**绝不进入渲染路径**：直接抛
    // not-cached（路由回 404，客户端把垫底图摘掉即可）。
    // 为什么必须有：静态帧是**取舍方案** —— 实时渲染能出好效果时，不该为一张过渡图
    // 跑 4K 冷渲染（worker + GPU，数百 ms~数秒）去和它抢 CPU/GPU（用户实测的卡顿 /
    // 首帧超时降级 / 黑屏链）。缓存里有就白得一张真帧，没有就空着等实时渲染。
    ...
    // 条目带等待者计数 + 取消信号: 这条路径会启动/ fork 渲染 worker
    // (单帧超时 600s), 客户端导航离开后旧实现无任何取消 → 子进程继续
    // 空转。全部等待者断开时 abort (renderSceneFrameInWorker 支持
    // opts.signal)。
    let entry = SCENE_FRAME_INFLIGHT.get(key);
    if (!entry) {
      const ctrl = new AbortController();
      const cancel = () => {
        if (ctrl.signal.aborted) return;
        ctrl.abort();
        // 立刻摘除: 取消后新到的请求要重新渲染, 不能加入已中止的任务。
        if (SCENE_FRAME_INFLIGHT.get(key) === entry) SCENE_FRAME_INFLIGHT.delete(key);
      };
      const pending = (async () => {
        const { extractSceneMainImage, extractSceneMainImageFromDir, isMainTextureUsable } = await import('./pkg-extract.js');
        // `.fb.` 兜底帧的可用性判据: 无产物 (质量门拒收 / 无候选) 或 **hasPuppet**。
        // 后者是因为 puppet 的材质贴图是**图集** —— 近似路径会把整张图集贴出去,
        // 画面出现部件网格。实测本地库 7/7 个 composite 场景都含 puppet 图集层,
        // 而 8 个单贴图场景全为 false ⇒ hasPuppet 精确等价于"该场景不能用近似"。
        // （全局"主纹理模式"入口已随 §6 退役；这里保留变量供真实渲染失败后的兜底复用。）
        let mainTexFrame = null;
        // 档 1/2 (上游 #91「壁纸画面刷新」的**替代来源**，UI 名「出图来源」): 直接用
        // 提取逻辑产出, **不经渲染 worker** —— 它们是自动链没有用到的两种更轻的来源。
        //   1 = 主纹理单张（跳过合成）, 2 = 作者原画优先
        // 档 3 = 作者预览图 / 4 = 自定义画面 不在此列（上面已短路）。
        if (variant === 1 || variant === 2) {
          const frame = abs.toLowerCase().endsWith('.json')
            ? extractSceneMainImageFromDir(dirname(abs), basename(abs), variant)
            : extractSceneMainImage(new Uint8Array(await readFile(abs)), basename(abs), variant);
          const target = frame.mime === 'image/jpeg' ? jpgPath : pngPath;
          await atomicWriteFileP(target, frame.bytes);
          return target;
        }
        ...
        try {
          ...
          // 静态帧时刻保持 2.5s: 一次性动画的"保持态"不靠推后时间实现, 而由渲染器
          // 依据壁纸自带数据 (属性动画 options.mode=single / MDL loop=single) 在
          // 单帧输出时直接取末值/末帧 (core.js _resolveAnimations 与 puppet.js
          // _sampleAnimRT 的 this.staticFrame 分支) —— 与 t 无关, 循环动画相位不变。
          let gpuAccel = false;
          try { gpuAccel = readSettings().sceneGpuAccel === true; } catch { /* 默认 CPU */ }
          const result = await renderSceneFrameInWorker(src, fw, fh, 2.5, { gpuAccel, signal: ctrl.signal });
          ...
          // 已取消 (最后一个等待者断开) 时不再走回退链: 主纹理提取同样是
          // 重活, 而且结果已无人接收。
          if (ctrl.signal.aborted) throw e;
          // 渲染失败(非 scene.pkg 结构/缺资源等) → 回退主纹理静态帧。
          // ⚠️ 回退必须留痕 (上游 issue #86): 缺 lib/scene-script-apis.js 时
          //   场景渲染器必然崩溃 → 每次请求都静默回退主纹理, 产出 8K 量级的
          //   巨图 (3830 万像素 / 146MB RGBA), 与"启动卡顿 / GPU 高占用"极易
          //   被误判为其它问题。原先这里完全静默, 只有体积对比才能发现。
          renderErr = e;
          try { unlinkSync(pngPath); } catch { /* ignore */ }
        }
        // 预热 (allowFallback=false): 到此为止 —— 不做回退提取 (慢且非真实产物)。
        // 不写 .fb., 于是路由后续仍会按原逻辑生成/复用兜底帧, 两边不互相污染。
        if (!allowFallback) {
          const err = new Error('render failed: ' + String(renderErr && renderErr.message ? renderErr.message : renderErr));
          err.renderFailed = true;
          throw err;
        }
        // 异步读盘：scene.pkg 可达几十 MB，readFileSync 会阻塞事件循环。
        // maintexture 模式下若已提取过 (因 hasPuppet 未采用), 直接复用, 避免重复提取。
        const frame = mainTexFrame || (abs.toLowerCase().endsWith('.json')
          ? extractSceneMainImageFromDir(dirname(abs), basename(abs), variant)
          : extractSceneMainImage(new Uint8Array(await readFile(abs)), basename(abs), variant));
        // 回退产物写独立路径 (.fb.), 不覆盖/不共用真实渲染的 key.png|jpg:
        // 真实渲染成功时下一请求按优先级命中它, 旧回退帧不再掩盖 (#86)。
        const target = frame.mime === 'image/jpeg' ? fbJpgPath : fbPngPath;
        await atomicWriteFileP(target, frame.bytes);
        gpuDiag('scene 渲染失败 → 已回退主纹理:',
          (abs.split(/[\\/]/).slice(-2).join('/')),
          'dims=', frame.width + 'x' + frame.height,
          // 留痕到实际写出的回退文件 (含 .fb. 标记, 与真实渲染产物可区分)
          'served=', target.split(/[\\/]/).pop(),
          'bytes=', Math.round(frame.bytes.length / 1048576) + 'MB',
          '原因=', String(renderErr && renderErr.message ? renderErr.message : renderErr));
        return target;
      })();
      entry = { promise: pending, waiters: 0, cancel };
      SCENE_FRAME_INFLIGHT.set(key, entry);
      // 无论成败都摘除（失败允许后续请求重试）；身份判定避免误删取消后
      // 新建的同 key 条目。
      pending.then(
        () => { if (SCENE_FRAME_INFLIGHT.get(key) === entry) SCENE_FRAME_INFLIGHT.delete(key); },
        () => { if (SCENE_FRAME_INFLIGHT.get(key) === entry) SCENE_FRAME_INFLIGHT.delete(key); },
    ...
    // 等待者登记必须在 await 之前: 渲染中客户端断开的 'close' 只发一次,
    // 之后再注册就永远收不到 → worker 跑满 600s 超时。
    entry.waiters++;
    let released = false;
    const release = () => {
      if (released) return;
    ...
    let fileAbs;
    try {
      fileAbs = await entry.promise;
    } finally {
      if (signal) signal.removeEventListener('abort', release);
    }
    return { fileAbs, servedFrom: fileAbs.endsWith('.fb.png') || fileAbs.endsWith('.fb.jpg') ? 'fallback' : 'render' };
  }

  // ── 排队预热 (G1) ─────────────────────────────────────────────────────────
  // 与路由共用 ensureSceneFrame ⇒ "预热帧与按需帧同键同内容"是结构保证。
  // 并发严格 = 1、空闲门控、失败退避都在队列内部 (见 lib/scene-prewarm.js)。
  const prewarmMru = [];           // 最近选中的场景 abs (最新在前)
  const PREWARM_MRU_MAX = 6;       // 默认预热范围: MRU 前 6 张
  let prewarmQueue = null;

  /** 从 frameUrl / sceneUrl / media 这类 URL 里取回宿主侧 abs 路径。 */
  function absFromMediaUrl(u) {
    if (typeof u !== 'string' || !u) return null;
    const m = /\/(?:scene-frame|media|scene-runtime|preview)\/([A-Za-z0-9_-]+)/.exec(u);
    if (!m) return null;
    return mediaMap.get(m[1]) || null;
  }

  /** 依据当前选中 + MRU (+ 可选全库) 计算 P1 候选名单, 交给队列。 */
  async function refreshPrewarmCandidates(currentAbs) {
    if (!prewarmQueue) return;
    const sceneOnly = (p) => typeof p === 'string' && /\.(pkg|json)$/i.test(p);
    // 当前**正在实时渲染**的壁纸不进候选名单：静态帧对它的角色只剩"切换回来的垫底图"，
    // 而它的帧恰恰是此刻最没必要算的一张（live 已经出画面了）。优先级让给"切换 / 轮换
    // 要用的那些"—— 这正是用户要的：动画优先，静态帧降级为缓存兜底。
    let skipCurrent = false;
    try { skipCurrent = Boolean(currentAbs) && liveSuppressesPrewarm(readSettings(), currentAbs); } catch { /* 保守：不跳过 */ }
    let list = [];
    if (currentAbs && !skipCurrent) list.push(currentAbs);
    for (const p of prewarmMru) if (p !== currentAbs) list.push(p);
    // 相邻项: 仅在 inventory 已缓存时取用 —— 不在设置热路径上强制触发全库扫描
    try {
      const cached = inventoryCache && (Date.now() - inventoryCache.t < INVENTORY_TTL_MS) ? inventoryCache.payload : null;
      const items = cached && Array.isArray(cached.wallpapers) ? cached.wallpapers : null;
      if (items && currentAbs) {
        const absOrder = items.map((w) => absFromMediaUrl(w.frameUrl)).filter(sceneOnly);
        const i = absOrder.indexOf(currentAbs);
        if (i >= 0) {
          for (const d of [1, -1, 2, -2]) {
            const p = absOrder[i + d];
            if (p && !list.includes(p)) list.push(p);
          }
        }
      }
      // 预热范围 = 更多: 全库场景都进候选 (受队列并发=1 与失败退避约束, 不会失控)
      if (items && prewarmScope() === 'all') {
        for (const w of items) {
          const p = absFromMediaUrl(w.frameUrl);
          if (p && !list.includes(p)) list.push(p);
        }
      }
    } catch { /* inventory 不可用 → 退化为仅 MRU */ }
    list = list.filter(sceneOnly);
    prewarmQueue.setCandidates(list.slice(0, prewarmScope() === 'all' ? 4096 : PREWARM_MRU_MAX + 4));
  }

  function prewarmScope() {
    try { return readSettings().scenePrewarmScope === 'all' ? 'all' : 'recent'; } catch { return 'recent'; }
  }

  function prewarmWanted() {
    try {
      const st = readSettings();
      // 「静态帧渲染」关掉后没有渲染产物可预热 —— 直接停摆，不等用户再想起这个开关。
      // ⚠️ **不**因为"实时渲染在用"而整体停摆：静态帧在这里的角色已降级为**切换 / 轮换的
      // 缓存兜底**，后台把它填好正是要的效果。实时渲染期间不去抢资源，靠下面两条实现：
      //   ① `/scene-live` + `/scene-files` 的流量算用户活动 ⇒ 预热推迟到动画起来之后；
      //   ② 当前**正在实时渲染**的那张不进候选名单（见 refreshPrewarmCandidates）。
      return st.scenePrewarm === true;
    } catch { return false; }
  }

  prewarmQueue = createPrewarmQueue({
    // 与上游提取式预热互斥（见 waitForExtractionPrewarm）：上游那轮在跑时本队列让路。
    ensure: async (abs, o) => {
      await waitForExtractionPrewarm(o && o.signal);
      return ensureSceneFrame(abs, o);
    },
    statePath: join(ensureFrameCacheDir(), 'prewarm-queue.json'),
    log: (...a) => gpuDiag('[prewarm]', ...a),
    enabled: prewarmWanted(),
  });
  prewarmQueue.start();
  // disposers 是按函数调用的 (见文件末尾 for..of d()), 必须 push 函数而非对象
  disposers.push(() => prewarmQueue.stop());
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 出图来源档位（?v=1..4，上游 #91 引入）：各档 = 不同**来源**，必须进缓存键；
      // 档 0/无参数沿用原键（既有帧零作废，见 ensureSceneFrame 的键兼容注释）。
      //   1 = 主纹理单张（跳过合成）· 2 = 作者原画优先
      //   7 = 作者内嵌 MP4 · 8 = 静态帧渲染（与档 0 **同槽**，见 sceneFrameCachePaths）
      //   4 = 自定义画面（用户截屏导入）
      //   3 = 作者预览图（作为回退来源已删除）、5 = 已退役（总开关曾误入档位表）、
      //   6 = 已退役（「合成」= 自动链自己的提取步，摆成手动档等于重试刚失败的那一步）
      // 以上退役/已删值以及任何越界值一律按 0（auto），避免留下语义已废的 _vN 缓存键。
      const vRaw = Number(new URL(req.url || '/', 'http://x').searchParams.get('v') || 0);
      const vParsed = Number.isFinite(vRaw) ? Math.floor(vRaw) : 0;
      const variant = clampFrameVariant(vParsed);
      // HEAD：纯磁盘探测（绝不触发渲染）—— live GPU 抓帧回填的客户端按它决定要不要
      // 抓帧：204 + X-WE-GPU（1 = 已有抓帧，0 = 只有 CPU 渲染/提取帧），404 = 空槽。
      // 已有抓帧时附 X-WE-GPU-W/H/AR（抓帧那一刻的设备像素几何，读 PNG 头），客户端
      // 据此判断这张帧还是不是当前视口的构图（不是就清掉按当前视口重抓）。
      if ((req.method || 'GET').toUpperCase() === 'HEAD') {
        ...
        const file = gpuFile || sceneFrameSlotFile(slot)
          || (existsSync(slot.fbPngPath) ? slot.fbPngPath : null)
          || (existsSync(slot.fbJpgPath) ? slot.fbJpgPath : null);
        if (!file) {
          res.statusCode = 404;
          res.setHeader('Content-Type', 'application/json; charset=utf-8');
      ...
      // 客户端断开 → abort 本等待者 (ensureSceneFrame 内按等待者计数决定是否取消渲染,
      // 语义与抽取前逐字一致: 多请求共享一次 fork, 最后一个离开才取消)。
      const ctrl = new AbortController();
      res.once('close', () => ctrl.abort());
      // 前台请求即用户活动 → 暂停后台预热 (队列内部按空闲计时判断)
      prewarmQueue.noteActivity();
      // `?cached=1` = **只命中、不渲染**（live 的垫底图 / 任何探测用）：缓存里有就
      // 白得一张真帧，没有就 404 让客户端空着 —— 静态帧是取舍方案，不该为过渡图
      // 抢实时渲染的 CPU/GPU（见 ensureSceneFrame 的 cachedOnly 注释）。
      const cachedOnly = (new URL(req.url || '/', 'http://x').searchParams.get('cached') || '') === '1';
      (async () => {
        const { fileAbs } = await ensureSceneFrame(abs, { signal: ctrl.signal, variant, cachedOnly });
        // 产物可能是 png/jpg/gif/webp —— 档 3 (作者预览图) 允许 gif, 必须给对 MIME。
        res.setHeader('Content-Type', fileAbs.endsWith('.jpg') ? 'image/jpeg'
          : fileAbs.endsWith('.gif') ? 'image/gif'
            : fileAbs.endsWith('.webp') ? 'image/webp' : 'image/png');
        res.setHeader('Cache-Control', 'no-store');
        trackStream(createReadStream(fileAbs), res);
      })().catch((err) => {
        // not-cached 是**正常**结果（探测未命中），不是渲染失败：给 404 而不是 422。
        res.statusCode = err && err.notCached ? 404 : 422;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.end(JSON.stringify({ error: String(err && err.message ? err.message : err) }));
      });
    },
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      let size = 0;
      let failed = false;
      let overLimit = false; // 超限后只排空、不再缓冲（见下面 data 处理器）
      const fail = (code, payload) => {
        ...
        // 但「应答写完就断」在 Windows 上仍会丢应答：此时对端还在发剩余请求体，
        // 服务端 socket 里有未读数据 → 断开发 RST → 客户端先报 ECONNRESET，刚收到
        // 的 413 被丢弃（本机实测 status=0 err=ECONNRESET）。改为**等对端发完**
        // （'end'）再断；排空走 resume()，不缓冲。对端不发完时 5s 兜底断开。
        const dropTimer = setTimeout(drop, 5000);
        if (typeof dropTimer.unref === 'function') dropTimer.unref();
        const finishThenDrop = () => {
          const done = () => { clearTimeout(dropTimer); drop(); };
          if (req.readableEnded) { done(); return; }
          if (typeof req.once === 'function') req.once('end', done);
          else done();
          try { req.resume(); } catch { /* ignore */ }
        };
        if (res.writableFinished) { finishThenDrop(); return; }
        if (typeof res.once === 'function') res.once('finish', finishThenDrop); else finishThenDrop();
        try { req.resume(); } catch { /* ignore */ }
      };
      armBodyIdleTimeout(req, () => fail(408, { error: 'request timeout' }));
      req.on('data', (c) => {
        if (failed || overLimit) return;
        size += c.length;
        if (size > GPU_FRAME_MAX_BYTES) {
          // 超限：立刻停止缓冲，但**等请求体排空后再回 413**。早回 + 立刻断会在服务端
          // 仍有未读数据时发 RST（Node 的 http server 在请求未读完就结束响应时也会主动
          // 断连），客户端先收到 ECONNRESET、把已经写出的 413 一起丢掉（本机实测
          // status=0 err=ECONNRESET）。排空不缓冲，代价只是把对端已发出的字节读完。
          // failed=true 同时让下面原有的 on('end') 直接返回（否则它会先回 415）。
          overLimit = true;
          failed = true;
          chunks.length = 0;
          let replied = false;
          ...
          const reply = () => {
            ...
            json(413, { error: 'frame too large' });
            const drop = () => { try { req.destroy(); } catch { /* ignore */ } };
            if (res.writableFinished) drop();
            else if (typeof res.once === 'function') res.once('finish', drop);
            else drop();
          };
          // 对端一直发不完时也要给出应答（本地回环上是防滥用路径，不能无限等）。
          timer = setTimeout(reply, 5000);
          if (typeof timer.unref === 'function') timer.unref();
          if (req.readableEnded) reply();
          else {
            if (typeof req.once === 'function') req.once('end', reply);
            try { req.resume(); } catch { /* ignore */ }
          }
          return;
        }
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 实时渲染页的流量（渲染页本身 + 它拉取的资源）**算用户活动** ⇒ 后台预热自动
      // 推迟 ≥ 一个空闲窗口。这就是"先保证动画渲染出画面，之后再跑后台静态帧渲染"：
      // 静态帧是取舍方案，绝不能在实时渲染首帧的窗口里抢 CPU/GPU（用户实测的卡顿 /
      // 首帧超时降级 / 黑屏链）。预热本身不受阻断，只是排在动画起来之后。
      if (prewarmQueue) prewarmQueue.noteActivity();
      let rest = '';
      try {
  ...
  function handleSceneFiles(req, res, mount) {
    ...
    // 与 /scene-live 同理：实时渲染的资源拉取算用户活动 ⇒ 后台渲染式预热推迟到
    // 动画起来之后（两套预热互斥见 waitForExtractionPrewarm）。
    if (prewarmQueue) prewarmQueue.noteActivity();
    const method = (req.method || 'GET').toUpperCase();
    if (method === 'OPTIONS') {
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      (async () => {
        ...
        if (!cachedMp4) {
          // in-flight 去重：同一 key 的并发请求共享一次提取 + 一次缓存写入
          // （与 SCENE_FRAME_INFLIGHT 同型: 条目 = {promise, waiters, cancel},
          // 全部等待者断开时 abort → 同步提取器在自然让出点抛 'cancelled'）。
          let entry = SCENE_VIDEO_INFLIGHT.get(key);
          if (!entry) {
            const ctrl = new AbortController();
            const cancel = () => {
              if (ctrl.signal.aborted) return;
              ctrl.abort();
              // 立刻摘除: 取消后新到的请求要重新提取, 不能加入已中止的任务。
              if (SCENE_VIDEO_INFLIGHT.get(key) === entry) SCENE_VIDEO_INFLIGHT.delete(key);
            };
            const pending = (async () => {
              try {
                // 异步读盘：scene.pkg 可达几十 MB，readFileSync 会阻塞事件循环。
                // 提取器是主线程同步解码 (无 worker), 只能靠 signal 在内部让出点取消。
                const bytes = abs.toLowerCase().endsWith('.json')
                  ? extractSceneVideoFromDir(dirname(abs), { signal: ctrl.signal })
                  : extractSceneVideo(new Uint8Array(await readFile(abs)), { signal: ctrl.signal });
                ...
                // 取消恰好落在写盘期间 (atomicWriteFileP 不响应 signal) 时产物已无人接收。
                if (ctrl.signal.aborted) throw new Error('cancelled');
                sceneVideoProbeSet(probeKey, true); // 产物已发布 → 正向回填
                return mp4Path;
              } catch (e) {
                // 取消路径不留残余产物: 提取器中止时尚未写盘 (unlink 为无害 no-op),
                // 写盘期间的取消则由上一行的检查抛出 → 这里删掉已发布的无主文件。
                if (ctrl.signal.aborted) { try { unlinkSync(mp4Path); } catch { /* ignore */ } }
                // 提取失败记否 (客户端 404 从此教会缓存)。但"被取消"不是"没有
                // 内嵌 MP4"的证据 (用户切走壁纸也会 abort), 记否会污染缓存 → 跳过。
                else sceneVideoProbeSet(probeKey, false);
                throw e;
              }
            })();
            entry = { promise: pending, waiters: 0, cancel };
            SCENE_VIDEO_INFLIGHT.set(key, entry);
            // 无论成败都摘除（失败允许后续请求重试）；身份判定避免误删取消后
            // 新建的同 key 条目。
            pending.then(
              () => { if (SCENE_VIDEO_INFLIGHT.get(key) === entry) SCENE_VIDEO_INFLIGHT.delete(key); },
          ...
          entry.waiters++;
          // 等待者登记必须在 await 之前: 提取中客户端断开的 'close' 只发一次,
          // 之后再注册就永远收不到 → 同步解码继续跑完没人要的结果。
          res.once('close', () => {
            entry.waiters--;
            if (entry.waiters <= 0) entry.cancel();
          });
          const produced = await entry.promise;
          if (!produced) {
            res.statusCode = 404;
            res.setHeader('Content-Type', 'application/json; charset=utf-8');
  ...
  disposers.push(webServer.register({
    ...
    handler: (req, res) => {
      ...
      req.on('end', () => {
        ...
        // 清单漂移的**可观测信号**：请求里带了、白名单却不认的键 ⇒ 三份清单里有哪份漏了。
        // 静默丢弃正是 0.7.5「画面档位永远存不住」难排查的原因（用户侧只看到"保存成功但下次就没了"），
        // 故这里显式记一行。另有构建期护栏 scripts/verify-settings-keys.mjs 做同等断言。
        try {
          const dropped = Object.keys(parsed).filter((k) => !(k in sanitized));
          if (dropped.length) {
            console.warn('[wallpaper-engine] settings PUT 丢弃了白名单外的键（客户端/宿主清单可能漂移）:',
              dropped.join(', '));
          }
        } catch { /* 自检异常绝不影响写入 */ }
        // 排队预热 (G1): 任何设置变更都算用户活动 ⇒ 暂停预热; 选中变化则提升到队首
        // 并刷新候选 (当前 + MRU 前 6, 范围=更多时纳入全库)。预热异常绝不影响设置写入。
        try {
          prewarmQueue.setEnabled(prewarmWanted());
          const curAbs = absFromMediaUrl(sanitized.url);
          if (curAbs) {
            const i = prewarmMru.indexOf(curAbs);
            if (i >= 0) prewarmMru.splice(i, 1);
            prewarmMru.unshift(curAbs);
            if (prewarmMru.length > PREWARM_MRU_MAX) prewarmMru.length = PREWARM_MRU_MAX;
            // 正在实时渲染的当前壁纸不提升（它不进候选名单，提升会让它插队渲染）。
            if (!liveSuppressesPrewarm(sanitized, curAbs)) prewarmQueue.promote(curAbs);
          }
          void refreshPrewarmCandidates(curAbs);
        } catch { /* 预热不得影响设置写入 */ }
  ...
  return () => {
    ...
    // 事件循环监控定时器: 归本次 apply 所有, 卸载 / HMR 时必须清掉, 否则每次
    // 重载都多一个永久 setInterval 并钉住旧模块实例 (旧实现模块级启动, 从不
    // 清理)。只清自己捕获的句柄, 不误伤后来者。
    if (evtLagTimer) {
      try { clearInterval(evtLagTimer); } catch { /* ignore */ }
      if (_evtLagStart === evtLagTimer) _evtLagStart = null;
    }
